...
cmake must be installed on build machine.

cmake version 3.4.3 or higher is required.
gcc 5.1 or higher is required.

Create build directory, in the examples we use '~/arbeit/habana/build' but it can be anywhere.
...
The build must create compiler executable 'tpc-clang' in the directory ~/arbeit/habana/build/bin'. It is used as usually, for instance, the next invocations may be used to compile source file to object, asm or llvm ir respectively:

~/arbeit/habana/build/bin/tpc-clang ~/arbeit/habana/tpc-llvm/clang/test/RC99/misc/reduction.c -c
~/arbeit/habana/build/bin/tpc-clang ~/arbeit/habana/tpc-llvm/clang/test/RC99/misc/reduction.c -S
~/arbeit/habana/build/bin/tpc-clang ~/arbeit/habana/tpc-llvm/clang/test/RC99/misc/reduction.c -S -emit-llvm

Specific compiler options
--------------------------

Optimization levels: -O2 and -O0 are max and min levels supported, -O2 is the default level.
...
-march=<name> 		Architecture switch, currently supported names are "goya", "gaudi", "gaudib", "greco", "gaudi2"

-max-tensors <n>	Tensor limit, n is a number in range 0..16. Default is architecture-dependent (8 for goya and 16 for others).
-vlm <n>		Vector local memory limit, n is amount of memory in KB. Default is 80KB if the program does not use LOOKUP instruction and 16KB otherwise. Beware that the compiler can need some VLM space for register spilling.
-slm <n>		Scalar local memory limit, n is amount of memory in KB. Default is architecture-dependent (1 for goya/gaudi, 2 for greco, 16 for gaudi2). Beware that the compiler can need some SLM space for register spilling.
-spill-vlm N		Limit VLM amount available for spills, abort compilation if this limit gets exceeded. If not set, whole VLM can be used.
...
-mllvm -ignore-mem-overflow=1 		Do not abort compilation if local memory limit is exceeded. Warning message is still issued.
...
-instr-compress		Force instruction compression (Greco and later architectures). Enabled for -O2 level by default and disabled for -O1/-O0.
-no-instr-compress	Disable instruction compression.
-long-irf		Enable extended IRF mode (44 bits per dimension). Default is standard mode (32 bit).
--tpcversion		Print TPC version information (git revision hash).
-disable-instruction-scheduling     Master flag to disable instruction scheduling and bundling, equivalent to "-mllvm -enable-misched=0 -mllvm -enable-post-misched=0 -mllvm -post-RA-scheduler=0 -mllvm -disable-tpc-packetizer -mllvm -tpc-trans-intr=0"
-mllvm -tpc-single-packets  Flag to force single instruction per VLIW bundle
...
__gaudi2__              architecture generation 4
...
Assembler
------------
Normally compiler handles assembler source files (*.tpcasm and *.s) the same way as c/cpp sources, no special care is needed. E.g.:
tpc-clang -c -march=goya my.s
In case you need to bypass common compilation driver and invoke assembler with extra options, here is an example:
clang -cc1as -triple tpc-none-none -target-cpu goya -filetype obj mytest.s
