...
#ifdef LLVM_TPC_COMPILER
extern cl::opt<bool> TPCCompressStatus;
extern cl::opt<std::string> TPCCompressStatusFile;
extern cl::opt<bool> TPCLeadingAddr;
extern cl::opt<bool> TPCEncodingInfo;
extern cl::opt<bool> TPCEncodingCover;
extern cl::opt<std::string> TPCEncodingCoverFile;
extern cl::opt<bool> IgnoreMovDT;
extern cl::opt<bool> NoCommonHeader;
#endif
...
void printTpcEncodingInfo(ArrayRef<uint8_t> instruction);
void printTpcEncodingInfo(const object::ObjectFile *O);
void TpcEncodingCover();
void calculateMCPU(const object::ObjectFile *ObjFile);
std::string getCPUName();
TPCMetadataSection getMetadataFromStringSection(const object::SectionRef& Section);
TPCMetadataSection getMetadataFromBinarySection(const object::SectionRef& Section);
void printTPCMetadataFromString(const object::SectionRef &Section);
void printTPCMetadataFromBinary(const object::SectionRef &Section);
void printIndexMap(const object::SectionRef &Section);
void printUnrollInfo(const object::SectionRef &Section);
void printBailoutInfo(const object::SectionRef &Section);
void collectSymbolsTpc(
    const object::ObjectFile *Obj, const MCDisassembler &DisAsm,
    MCContext &Context,
