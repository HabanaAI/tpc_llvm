...
#include "TPCDump.h" // LLVM_TPC_COMPILER
...
/*LLVM_TPC_COMPILER static*/ const Target *llvm::getTarget(const ObjectFile *Obj) {
...
static void disassembleObject(const Target *TheTarget, const ObjectFile *Obj,
  ...
  if (AllSymbols.empty() && Obj->isELF() && isTPCTarget(Obj))
    collectSymbolsTpc(Obj, *DisAsm, Ctx, AbsoluteSymbols);
  else
#endif
  ...
  for (const SectionRef &Section : ToolSectionFilter(*Obj)) {
...
#ifdef LLVM_TPC_COMPILER
    if (tryDisassembleTPCSection(Section))
      continue;
#endif
...
#ifdef LLVM_TPC_COMPILER
    SectionSymbolsTy &Symbols = (isTPCTarget(Obj) && Section.isText() &&
                                 AllSymbols[Section].size()==0) ? AbsoluteSymbols : AllSymbols[Section];
#else
...
#ifdef LLVM_TPC_COMPILER
    if (Obj->isELF() &&
        (Obj->getArch() == Triple::amdgcn || isTPCTarget(Obj))) {
#else
...
#ifdef LLVM_TPC_COMPILER
    PrintedSection = !printCommonHeader(Obj);
#endif
    ...
    for (unsigned SI = 0, SE = Symbols.size(); SI != SE; ++SI) {
...
#ifdef LLVM_TPC_COMPILER
      if (isTPCTarget(Obj) && (Start == End)) {
        if (Symbols[SI].Type == ELF::STT_NOTYPE) {
          // print multiple labels to the same location
          // useful for re-assembling support
          outs() << SymbolName << ":\n";  
        }
        continue;
      }
      ...
      if (Obj->isXCOFF() && SymbolDescription) {
        ...
        outs() << '<' << SymbolName << ">:\n";
#ifdef LLVM_TPC_COMPILER
        } else if (printAsTPCSymbol(Section, Symbols[SI]))
          outs() << SymbolName << ":\n";
#endif
      ...
      while (Index < End) {
        ...
        if (DumpARMELFData) {
...
#ifdef LLVM_TPC_COMPILER
          printTpcEncodingInfo(Obj, Bytes.slice(Index, Size), FOS);
#endif
          ...
          // Bundled instructions may leak memory:
          // 1) Lenghty instruction allocates operands storage outside placement
          // MCContext thus becomes not trivially destructable, 2) But nested
          // (i.e. referenced via bundle operands) instructions are not destroyed
          // along with the bundle. In this case, do the cleanup manually
          if (Disassembled) {
            auto DeepClean = [](auto &Clean, MCInst &I) -> void {
              for (MCOperand &Op : I) {
                if (Op.isInst() /* && Op.getInst()->getNumOperands() > 8 */) {
                  auto &BI = *const_cast<MCInst *>(Op.getInst());
                  Clean(Clean, BI);
                  BI.~MCInst();
                }
              }
            };
            DeepClean(DeepClean, Inst);
          }
        }
...
#ifdef LLVM_TPC_COMPILER
  // Check existing tpc metadata if target is "tpc"
  // So fix MCPU if it is nessesary
  calculateMCPU(Obj);
#endif
...
static void dumpObject(ObjectFile *O, const Archive *A = nullptr,
  ...
  if (!RawClangAST && printCommonHeader(O)) {
    outs() << '\n';
    if (A)
...
#ifdef LLVM_TPC_COMPILER
  if (TpcEncodingCover(TripleName))
    return EXIT_SUCCESS;
#endif
