...
static void disassembleObject(const Target *TheTarget, const ObjectFile *Obj,
  ...
  if (AllSymbols.empty() && Obj->isELF())
    ...
    if (getTarget(Obj)->getName() != StringRef("tpc"))
  ...
  for (const SectionRef &Section : ToolSectionFilter(*Obj)) {
    ...
    bool IsIndexMap =
        *SectionNameOrError == ".IndexMap" &&
        Obj->isELF() && getTarget(Obj)->getName() == StringRef("tpc");
    bool IsUnrollInfo = *SectionNameOrError == ".UnrollInfo" && Obj->isELF() &&
                        getTarget(Obj)->getName() == StringRef("tpc");
    bool IsBailout = *SectionNameOrError == ".BailoutGCCUSTOM" &&
                     Obj->isELF() &&
                     getTarget(Obj)->getName() == StringRef("tpc");
    bool IsBinaryTPCMetadata =
        *SectionNameOrError == BinaryTPCMetadataSectionName &&
        Obj->isELF() && getTarget(Obj)->getName() == StringRef("tpc");
    if (FilterSections.empty() && !DisassembleAll &&
        (!Section.isText() || Section.isVirtual()) && !IsBinaryTPCMetadata &&
        !IsIndexMap && !IsUnrollInfo && !IsBailout)
#else
    if (FilterSections.empty() && !DisassembleAll &&
...
#ifdef LLVM_TPC_COMPILER
    SectionSymbolsTy &Symbols = (getTarget(Obj)->getName()==StringRef("tpc")
                                && Section.isText()
    ...
    if (Obj->isELF() &&
        (Obj->getArch() == Triple::amdgcn ||
         getTarget(Obj)->getName() == StringRef("tpc"))) {
      // TPC and AMDGPU disassembler use symbolizer for printing labels
#else
...
#ifdef LLVM_TPC_COMPILER
    PrintedSection = NoCommonHeader;
    if (!NoCommonHeader && getTarget(Obj)->getName() == StringRef("tpc")) {
      outs() << "\nTPC architecture: " << getCPUName() << ".\n";
    }
#endif
    ...
    for (unsigned SI = 0, SE = Symbols.size(); SI != SE; ++SI) {
      ...
      if (Start > End || End <= StartAddress ||
          (getTarget(Obj)->getName() != StringRef("tpc") && (Start == End)))
#else
      if (Start >= End || End <= StartAddress)
#endif
        continue;
      ...
      if (getTarget(Obj)->getName() == StringRef("tpc") &&
        ...
        continue;
      } else if (getTarget(Obj)->getName() == StringRef("tpc") &&
                 IsIndexMap) {
        printIndexMap(Section);
        continue;
      } else if (getTarget(Obj)->getName() == StringRef("tpc") &&
                 IsUnrollInfo) {
        printUnrollInfo(Section);
        continue;
      } else if (getTarget(Obj)->getName() == StringRef("tpc") && IsBailout) {
        printBailoutInfo(Section);
        continue;
      }
      ...
      if (getTarget(Obj)->getName() != StringRef("tpc") ||
          (getTarget(Obj)->getName() == StringRef("tpc") &&
           !(SymbolName == SectionName &&
             std::get<0>(Symbols[SI]) == SectionAddr &&
             NoCommonHeader)))
#endif
      outs() << SymbolName << ":\n";
      ...
      while (Index < End) {
        ...
        if (getTarget(Obj)->getName() == StringRef("tpc") &&
            TPCEncodingInfo)
          printTpcEncodingInfo(Bytes.slice(Index, Size));
#endif
...
static void disassembleObject(const ObjectFile *Obj, bool InlineRelocs) {
  ...
  std::unique_ptr<const MCSubtargetInfo> STI(
      TheTarget->createMCSubtargetInfo(
          TripleName,
#ifdef LLVM_TPC_COMPILER
          getTarget(Obj)->getName() == StringRef("tpc") ? getCPUName() : MCPU,
#else
          MCPU,
#endif
          Features.getString()));
...
static void dumpObject(ObjectFile *O, const Archive *A = nullptr,
  ...
  if (TPCCompressStatus)
    printTpcCompressStatus(O);
  if (TPCEncodingInfo && !Disassemble)
    printTpcEncodingInfo(O);
#endif
}
