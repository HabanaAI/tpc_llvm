...
template <class G>
void AbstractDependenceGraphBuilder<G>::createAndConnectExitNode() {
  auto &ExitNode = createExitNode();
  for (auto *N : Graph) {
    if (*N == ExitNode)
      continue;
    if (!isa<SimpleDDGNode>(*N))
      continue;
    bool FoundExiting = false;
    for (const Instruction *I :
         cast<const SimpleDDGNode>(*N).getInstructions()) {
      if (isa<ReturnInst>(I)) {
        FoundExiting = true;
        break;
      }
    ...
    for (Instruction *II : SrcIList) {
      for (User *U : II->users()) {
        ...
        if (VisitedTargets.insert(DstNode).second) {
          ...
          if (shouldCreateUseDefEdges()) {
            createUseDefEdge(*DstNode, *N);
          }
          ++TotalDefUseEdges;
        }
      }
