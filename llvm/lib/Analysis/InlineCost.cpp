  ...
  for (Function::iterator BI = F.begin(), BE = F.end(); BI != BE; ++BI) {
    ...
    for (auto &II : *BI) {
      ...
      if (&F == Call->getCalledFunction()) {
        ...
        bool IsTpcTarget = false;
        IsTpcTarget = (Triple.size() > 3 && Triple.substr(0, 4) == "tpc-") ||
                      (Triple.size() == 3 && Triple == "tpc");
        if (IsTpcTarget) {
          std::string ErrMsg = Call->getCalledFunction()->getName();
          report_fatal_error("Function '" + ErrMsg + "' participates in recursion call.");
        }
