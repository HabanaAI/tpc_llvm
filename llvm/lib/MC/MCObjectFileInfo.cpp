...
void MCObjectFileInfo::initELFMCObjectFileInfo(const Triple &T, bool Large) {
  ...
  if (T.getArch() == Triple::ArchType::tpc) {
    // TPC specific sections.
    SouceCodeSection = Ctx->getELFSection(".source", ELF::SHT_PROGBITS, 0);
    LlvmIrSection = Ctx->getELFSection(".llvmir", ELF::SHT_PROGBITS, 0);
    ScalarLocalsSection = Ctx->getELFSection(".sldata", ELF::SHT_NOBITS,
                                             ELF::SHF_WRITE | ELF::SHF_ALLOC);
    VectorLocalsSection = Ctx->getELFSection(".vldata", ELF::SHT_NOBITS,
                                             ELF::SHF_WRITE | ELF::SHF_ALLOC);
    CommentSection = Ctx->getELFSection(
