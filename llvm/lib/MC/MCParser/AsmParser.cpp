...
bool AsmParser::parseStatement(ParseStatementInfo &Info,
  ...
  if (Lexer.is(AsmToken::EndOfStatement)) {
    ...
    {
      const Triple &Trip = getContext().getObjectFileInfo()->getTargetTriple();
      if (Trip.getArch() == Triple::tpc) {
        getTargetParser().onEndOfStatement();
        Out.AddBlankLine();
      }
      else {
        Out.AddBlankLine();
  ...
  AsmToken ID = getTok();
#ifdef LLVM_TPC_COMPILER
  if (ID.getString().equals("||")) {
    // New assembler format for TPC target.
    // Therefore this token is valid and has to be ignored.
    Lex();
    return true;
  }
#endif
