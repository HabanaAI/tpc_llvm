...
static Attribute::AttrKind getAttrFromCode(uint64_t Code) {
  ...
  case bitc::ATTR_KIND_INDEX_SPACE:
    return Attribute::IndexSpace;
  case bitc::ATTR_KIND_INDEX_SPACE_B:
    return Attribute::IndexSpaceb;
  case bitc::ATTR_KIND_REDUCTION_OR_NORM_AXES:
    return Attribute::ReductionOrNormAxes;
  case bitc::ATTR_KIND_UNROLL_INFO:
    return Attribute::UnrollInfo;
  }
}
...
Error BitcodeReader::parseTypeTableBody() {
  ...
  while (true) {
    ...
    case bitc::TYPE_CODE_FLOAT8_143:
      ResultTy = Type::getF8_143Ty(Context);
      break;
    case bitc::TYPE_CODE_FLOAT8_152:
      ResultTy = Type::getF8_152Ty(Context);
      break;
    case bitc::TYPE_CODE_BFLOAT16:
      ResultTy = Type::getBFloat16Ty(Context);
      break;
#endif
...
#ifdef LLVM_TPC_COMPILER
      if (CurTy->isF8_143Ty())
        V = ConstantFP::get(Context, APFloat(APFloat::Float8_143(),
                                             APInt(8, (uint16_t)Record[0])));
      else if (CurTy->isF8_152Ty())
        V = ConstantFP::get(Context, APFloat(APFloat::Float8_152(),
                                             APInt(8, (uint16_t)Record[0])));
      else if (CurTy->isBFloat16Ty())
        V = ConstantFP::get(Context, APFloat(APFloat::BFloat16(),
