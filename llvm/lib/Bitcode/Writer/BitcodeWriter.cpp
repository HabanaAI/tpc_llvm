...
static uint64_t getAttrKindEncoding(Attribute::AttrKind Kind) {
  ...
  case Attribute::IndexSpace:
    return bitc::ATTR_KIND_INDEX_SPACE;
  case Attribute::IndexSpaceb:
    return bitc::ATTR_KIND_INDEX_SPACE_B;
  case Attribute::ReductionOrNormAxes:
    return bitc::ATTR_KIND_REDUCTION_OR_NORM_AXES;
  case Attribute::UnrollInfo:
    return bitc::ATTR_KIND_UNROLL_INFO;
...
void ModuleBitcodeWriter::writeTypeTable() {
  ...
  for (unsigned i = 0, e = TypeList.size(); i != e; ++i) {
    ...
    case Type::F8_143ID:      Code = bitc::TYPE_CODE_FLOAT8_143; break;
    case Type::F8_152ID:      Code = bitc::TYPE_CODE_FLOAT8_152; break;
    case Type::BFloat16ID:    Code = bitc::TYPE_CODE_BFLOAT16;  break;
#endif
...
void ModuleBitcodeWriter::writeConstants(unsigned FirstVal, unsigned LastVal,
  ...
  for (unsigned i = FirstVal; i != LastVal; ++i) {
    ...
    if (C->isNullValue()) {
      ...
      Type *Ty = CFP->getType();
#ifdef LLVM_TPC_COMPILER
      if (Ty->isHalfTy() || Ty->isBFloat16Ty() || Ty->isFloatTy() ||
          Ty->isF8_143Ty() || Ty->isF8_152Ty() || Ty->isDoubleTy()) {
#else
