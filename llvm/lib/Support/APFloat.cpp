...
namespace llvm {
  ...
  static const fltSemantics semFloat8_143 = { 7, -6, 4, 8 };
  static const fltSemantics semFloat8_152 = { 15, -14, 3, 8 };
  static const fltSemantics semBFloat16 = { 127, -126, 8, 16 };
#endif
...
#ifdef LLVM_TPC_COMPILER
    if (&Sem == &llvm::APFloat::Float8_143())
      return S_Float8_143;
    if (&Sem == &llvm::APFloat::Float8_152())
      return S_Float8_152;
    if (&Sem == &llvm::APFloat::BFloat16())
      return S_BFloat16;
#endif
...
#ifdef LLVM_TPC_COMPILER
  const fltSemantics &APFloatBase::Float8_143() {
    return semFloat8_143;
  }
  const fltSemantics &APFloatBase::Float8_152() {
    return semFloat8_152;
  }
  const fltSemantics &APFloatBase::BFloat16() {
    return semBFloat16;
  }
#endif
...
#ifdef LLVM_TPC_COMPILER
APInt IEEEFloat::convertFloat8_143APFloatToAPInt() const {
  assert(semantics == (const llvm::fltSemantics *)&semFloat8_143);
  assert(partCount() == 1);

  uint8_t myexponent, mysignificand;

  if (isFiniteNonZero()) {
    myexponent = exponent + 7; //bias
    mysignificand = (uint8_t)*significandParts();
    if (myexponent == 1 && !(mysignificand & 0x08))
      myexponent = 0;   // denormal
  } else if (category == fcZero) {
    myexponent = 0;
    mysignificand = 0;
  ...
  return APInt(16, (((sign & 1) << 7) | ((myexponent & 0xf) << 3) |
                    (mysignificand & 0x07)));
}

APInt IEEEFloat::convertFloat8_152APFloatToAPInt() const {
  assert(semantics == (const llvm::fltSemantics *)&semFloat8_152);
  assert(partCount() == 1);
  ...
  if (isFiniteNonZero()) {
    myexponent = exponent + 15; //bias
    mysignificand = (uint8_t)*significandParts();
    if (myexponent == 1 && !(mysignificand & 0x04))
      myexponent = 0;   // denormal
  } else if (category == fcZero) {
    myexponent = 0;
    mysignificand = 0;
  ...
  return APInt(16, (((sign & 1) << 7) | ((myexponent & 0x1f) << 2) |
                    (mysignificand & 0x03)));
}

APInt IEEEFloat::convertBFloat16APFloatToAPInt() const {
  assert(semantics == (const llvm::fltSemantics*)&semBFloat16);
  assert(partCount() == 1);

  uint16_t myexponent, mysignificand;

  if (isFiniteNonZero()) {
    myexponent = exponent + 127; //bias
    mysignificand = (uint16_t)*significandParts();
...
#ifdef LLVM_TPC_COMPILER
  if (semantics == (const llvm::fltSemantics*)&semBFloat16)
    return convertBFloat16APFloatToAPInt();
  if (semantics == (const llvm::fltSemantics*)&semFloat8_143)
    return convertFloat8_143APFloatToAPInt();
  if (semantics == (const llvm::fltSemantics*)&semFloat8_152)
    return convertFloat8_152APFloatToAPInt();
#endif
...
#ifdef LLVM_TPC_COMPILER
void IEEEFloat::initFromFloat8_143APInt(const APInt &api) {
  assert(api.getBitWidth() == 8);
  uint8_t i = (uint8_t)*api.getRawData();
  uint8_t myexponent = (i >> 3) & 0x0f;
  uint8_t mysignificand = i & 0x7;

  initialize(&semFloat8_143);
  assert(partCount() == 1);
  ...
  if (myexponent == 0 && mysignificand == 0) {
    // exponent, significand meaningless
    category = fcZero;
  } else if (myexponent == 0x0f && mysignificand == 0) {
    // exponent, significand meaningless
    category = fcInfinity;
  } else if (myexponent == 0x0f && mysignificand != 0) {
    // sign, exponent, significand meaningless
    ...
    *significandParts() = mysignificand;
    if (myexponent == 0)    // denormal
      ...
      *significandParts() |= 0x08; // integer bit
  }
}

void IEEEFloat::initFromFloat8_152APInt(const APInt &api) {
  assert(api.getBitWidth() == 8);
  uint8_t i = (uint8_t)*api.getRawData();
  uint8_t myexponent = (i >> 2) & 0x1f;
  uint8_t mysignificand = i & 0x3;

  initialize(&semFloat8_152);
  assert(partCount() == 1);
  ...
  if (myexponent == 0 && mysignificand == 0) {
    ...
    *significandParts() = mysignificand;
    if (myexponent == 0)    // denormal
      ...
      *significandParts() |= 0x04; // integer bit
  }
}

void IEEEFloat::initFromBFloat16APInt(const APInt &api) {
  assert(api.getBitWidth() == 16);
  uint16_t i = (uint16_t)*api.getRawData();
  uint16_t myexponent = (i >> 7) & 0xff;
  uint16_t mysignificand = i & 0x7f;

  initialize(&semBFloat16);
  assert(partCount() == 1);
...
#ifdef LLVM_TPC_COMPILER
  if (Sem == &semFloat8_143)
    return initFromFloat8_143APInt(api);
  if (Sem == &semFloat8_152)
    return initFromFloat8_152APInt(api);
  if (Sem == &semBFloat16)
    return initFromBFloat16APInt(api);
#endif
  if (Sem == &semIEEEhalf)
