...
#include "llvm/Transforms/Scalar/CoordUpdateSimplifyPass.h"
...
static cl::opt<bool>
    EnableLoopSWPPass("enable-loop-swp-pass", cl::init(true), cl::Hidden,
                      cl::desc("Enable the loop software pipelining pass."));

static cl::opt<bool> EnableCastSwizzleOpt(
    "enable-cast-swizzle-opt", cl::init(false), cl::Hidden,
    cl::desc("Enables the cast swizzle optimization pass."));

static cl::opt<bool> EnableIterationInterleave(
    "enable-iter-ilv", cl::init(false), cl::Hidden,
    cl::desc("Enables the interleave iteration pass."));

static cl::opt<bool> EnableCoordUpdateSimplifyPass(
    "enable-simplify-coord-update-pass", cl::init(true), cl::Hidden,
    cl::desc("Enable the Coordinate update simplification pass."));
#endif
...
PassManagerBuilder::PassManagerBuilder() {
    ...
    RunLoopSWPPass = EnableLoopSWPPass;
    CastSwizzleOpt = EnableCastSwizzleOpt;
    IterationInterleave = EnableIterationInterleave;
    CoordUpdateSimplify = EnableCoordUpdateSimplifyPass;
}
...
void PassManagerBuilder::addFunctionSimplificationPasses(
...
#ifdef LLVM_TPC_COMPILER
  if (IsTPCTarget)
    MPM.add(createEvalSpecialFunctionPass());
#endif
  ...
  if (EnableSimpleLoopUnswitch)
    ...
    MPM.add(createLoopUnswitchPass(SizeLevel || (OptLevel < 3 && !IsTPCTarget), DivergentTarget));
...
#ifdef LLVM_TPC_COMPILER
  if (IsTPCTarget) {
    MPM.add(createTPCReplaceCmpSelPass());
    if (CastSwizzleOpt)
      MPM.add(createEliminateSwizzleCastLegacyPass());
    if (CoordUpdateSimplify)
      MPM.add(createCoordUpdateSimplifyPass());
    MPM.add(createLoopSimplifyCFGPass());
  }
#endif
...
void PassManagerBuilder::populateModulePassManager(
...
#ifdef LLVM_TPC_COMPILER
  if (IsTPCTarget)
    MPM.add(createLinkTPCHeadersLegacyPass());
#endif
...
#endif
  MPM.add(createLoopRotatePass(SizeLevel == 2 ? 0 : -1));
  if (!DisableUnrollLoops && EnableUnrollAndJam) {
    MPM.add(createLoopTakenPass(OptLevel));
    addFunctionSimplificationPasses(MPM);
  ...
  if (EnableUnrollAndJam && !DisableUnrollLoops) {
    ...
    // Move unroll and jam before loop distribute pass.
    // MPM.add(createLoopUnrollAndJamPass(OptLevel));
  }
...
#ifdef LLVM_TPC_COMPILER
  if (IsTPCTarget && RunLoopSWPPass)
    MPM.add(createLoopSWPPass());
#endif
...
#ifdef LLVM_TPC_COMPILER
  if (OptLevel > 1) {
    if (IsTPCTarget && IterationInterleave) {
      MPM.add(createLoopSimplifyCFGPass());
      MPM.add(createIterationInterleavePass());
    }
  }
#endif
