...
static bool isPaddingRegion(AllocaInst &AI, AllocaSlices &AS,
                            Partition &P) {
  if (auto STy = dyn_cast<StructType>(AI.getAllocatedType())) {
    const DataLayout &DL = AI.getModule()->getDataLayout();
    uint64_t Offset = P.beginOffset();
    ...
    if (ElementTy->isAggregateType())
      return false;
    unsigned PrimitiveElementBitSize = ElementTy->getScalarSizeInBits();
    if (PrimitiveElementBitSize == 0)
      return false; // Not primitive type
    if (8 * Offset == PrimitiveElementBitSize && Offset + Size == ElementSize) {
      // This partition represents alignment padding.
      return true;
    }
  }
  return false;
...
AllocaInst *SROA::rewritePartition(AllocaInst &AI, AllocaSlices &AS,
                                   Partition &P) {
#ifdef LLVM_TPC_COMPILER
  const Module* M = AI.getModule();
  Triple TargetTriple(M->getTargetTriple());
  if (TargetTriple.getArch() == Triple::tpc) {
    if (isPaddingRegion(AI, AS, P))
      return nullptr;
  }
#endif
