...
static cl::opt<unsigned> TPCUnrollCount(
    "tpc-unroll-count", cl::init(0), cl::Hidden,
    cl::desc("On TPC, use this unroll count for inner most loop "
             "unrolling.It is disabled by default"));
...
#ifdef LLVM_TPC_COMPILER
  // Check if the target is "TPC"
  Triple TargetTriple(
      L->getHeader()->getParent()->getParent()->getTargetTriple());
  bool IsTpcTarget = TargetTriple.getArch() == Triple::tpc;

  // Find the depth of inner most loop
  unsigned Depth = 0;
  for (Loop::iterator I = L->begin(), E = L->end(); I != E; ++I) {
    ++Depth;
  }

  // Check if user has specified "tpc-unroll-count"
  bool UserTPCUnrollCount = TPCUnrollCount.getNumOccurrences() > 0;
#endif
...
#ifdef LLVM_TPC_COMPILER
  if (UserUnrollCount && !UserTPCUnrollCount) {
#else
  if (UserUnrollCount) {
#endif
...
#ifdef LLVM_TPC_COMPILER
  bool IsConstBounds = false;
  Optional<Loop::LoopBounds> LB = L->getBounds(SE);
  if (LB) {
    LLVM_DEBUG(dbgs() << "Checking bounds... \n");
    Value &Initial = LB->getInitialIVValue();
    Value &Final = LB->getFinalIVValue();
    auto *CI = dyn_cast<ConstantInt>(&Initial);
    auto *CF = dyn_cast<ConstantInt>(&Final);
    if (CI || CF)
      IsConstBounds = true;
  }
  // On TPC target for non constant bounds, handle TPC specific unroll count
  // through pragma path which also restrict the unrolling to innermost loop
  // only by checking 'Depth is 0'.Setting TPCUnrollCount to 0 disables this
  // path.
  if (!IsConstBounds && IsTpcTarget && Depth == 0 && TPCUnrollCount > 0) {
    PragmaCount = TPCUnrollCount;
    TripMultiple = PragmaCount;
  }
#endif
...
#ifdef LLVM_TPC_COMPILER
static void setLoopUnrollMetadata(Function *F, unsigned UnrollCount,
                                  int LoopDepth) {

  MDNode *MD = F->getMetadata("unroll_info");
  LLVMContext &Ctx = F->getContext();
  std::string UnrollInfoStr = "{" + std::to_string(LoopDepth) + ", " +
                              std::to_string(UnrollCount) + "}";
  SmallVector<llvm::Metadata *, 16> UnrollInfoMDArgs;
  UnrollInfoMDArgs.push_back(MDString::get(Ctx, StringRef(UnrollInfoStr)));

  if (!MD) {
    F->setMetadata("unroll_info", MDNode::get(Ctx, UnrollInfoMDArgs));
  } else {
    SmallVector<llvm::Metadata *, 16> MDArgs;
    for (Metadata *Arg : MD->operands())
      MDArgs.push_back(Arg);
    MDArgs.append(UnrollInfoMDArgs.begin(), UnrollInfoMDArgs.end());
    F->setMetadata("unroll_info", MDNode::get(Ctx, MDArgs));
  }
}
#endif
...
static LoopUnrollResult tryToUnrollLoop(
  ...
  int LoopDepth = L->getLoopDepth() - 1;
#endif
  ...
  if (IsTpcTarget) {
    ...
    if (UnrollResult == LoopUnrollResult::PartiallyUnrolled)
      UnrollCount = UP.Count;
    else if (UnrollResult == LoopUnrollResult::Unmodified)
      UnrollCount = 1;
    else if (UnrollResult == LoopUnrollResult::FullyUnrolled)
      UnrollCount = TripCount;

    setLoopUnrollMetadata(Func, UnrollCount, LoopDepth);
  }
  ...
  if (UnrollResult == LoopUnrollResult::PartiallyUnrolled) {
    ...
    // For the inner most loop, perform scaling of loop bounds with the unroll
    // factor.
    if (IsTpcTarget && Depth == 0 && TPCUnrollCount > 0) {

      // Get loop bounds.
      Optional<Loop::LoopBounds> LB = L->getBounds(SE);
      if (LB) {
        LLVM_DEBUG(dbgs() << "Performing bounds scaling... \n");
        Value &Initial = LB->getInitialIVValue();
        Value &Final = LB->getFinalIVValue();
        LLVM_DEBUG(dbgs() << "Initial : " << Initial << "\n");
        LLVM_DEBUG(dbgs() << "Final : " << Final << "\n");
        Instruction *II = dyn_cast<Instruction>(&Initial);
        Instruction *IF = dyn_cast<Instruction>(&Final);

        // Insert scaling of loop lower bound
        Type *Ty = Initial.getType();
        unsigned BitWidth = Ty->getScalarSizeInBits();
        Instruction *ClonedI = II->clone();
        bool IsVoidRetTy = II->getType()->isVoidTy();
        if (!IsVoidRetTy)
          ClonedI->setName(II->getName() + ".cloned");
        Instruction *I = BinaryOperator::CreateMul(
            ClonedI, ConstantInt::get(Ty, APInt(BitWidth, TPCUnrollCount)));

        if (PHINode *PN = dyn_cast<PHINode>(II)) {
          auto *BB = II->getParent();
          auto InsertionPt = BB->getFirstInsertionPt();
          ClonedI->insertBefore(&*InsertionPt);
        } else {
          ClonedI->insertAfter(II);
        }
        I->insertAfter(ClonedI);
        II->replaceAllUsesWith(I);

        // Insert scaling of loop upper bound
        Ty = Final.getType();
        BitWidth = Ty->getScalarSizeInBits();
        Instruction *ClonedF = IF->clone();
        IsVoidRetTy = IF->getType()->isVoidTy();
        if (!IsVoidRetTy)
          ClonedF->setName(IF->getName() + ".cloned");
        I = BinaryOperator::CreateMul(
            ClonedF, ConstantInt::get(Ty, APInt(BitWidth, TPCUnrollCount)));
        ClonedF->insertAfter(IF);
        I->insertAfter(ClonedF);
        IF->replaceAllUsesWith(I);
      }
    }
  }
#endif
  ...
  if (RemainderLoop) {
    ...
    addStringMetadataToLoop(RemainderLoop,
