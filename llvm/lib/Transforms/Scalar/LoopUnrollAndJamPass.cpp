...
static cl::opt<bool> IgnoreUnrollAndJamRemainder(
    "ignore-unroll-and-jam-remainder", cl::Hidden,
    cl::desc("Ignore remainder of loop to be unroll-and-jammed."));

static cl::opt<bool> UnrollAndJamAssumeSafety(
    "unroll-and-jam-assume-safety", cl::Hidden,
    cl::desc("Assume safety for loops to be unroll-and-jammed."));
...
tryToUnrollAndJamLoop(Loop *L, DominatorTree &DT, LoopInfo *LI,
  ...
  removeFalseDependence(L);

  if (!isSafeToUnrollAndJam(L, SE, DT, DI, UnrollAndJamAssumeSafety)) {
  ...
  LoopUnrollResult UnrollResult = UnrollAndJamLoop(
      ...
      &SE, &DT, &AC, &ORE, IgnoreUnrollAndJamRemainder, &EpilogueOuterLoop);
