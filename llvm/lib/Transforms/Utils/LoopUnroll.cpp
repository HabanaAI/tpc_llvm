...
static bool skipLoopRemander(const Loop *L) {
  return GetUnrollMetadataForLoop(L, "llvm.loop.skip_remainder.enable");
}
#endif
...
LoopUnrollResult llvm::UnrollLoop(Loop *L, UnrollLoopOptions ULO, LoopInfo *LI,
  ...
  bool for_tpc = true;
#ifdef LLVM_TPC_COMPILER
  Function *F = (*L->block_begin())->getParent();
  Module * M = F->getParent();
  assert(M);
  Triple TargetTriple(M->getTargetTriple());
  for_tpc = TargetTriple.getArch() != Triple::tpc || !(skipLoopRemander(L) && !RuntimeTripCount);
#endif
