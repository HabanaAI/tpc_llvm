...
MachineLICMBase::CanCauseHighRegPressure(const DenseMap<unsigned, int>& Cost,
  ...
  for (const auto &RPIdAndCost : Cost) {
    ...
    // If the register pressure limit is 2, it means the register class has
    // only one available register.
    // Check whether the loop is going to be unrolled. If it is, the instruction
    // will be cloned and we will get unavoidable spills inside the loop.
    // So we try to hoist the instruction before unrolling disregarding register pressure.
    // Even if it'll create spills at least they're gonna be outside the loop
    const Module* M = CurLoop->getHeader()->getParent()->getMMI().getModule();
    Triple TargetTriple(M->getTargetTriple());
    if (Limit <= 2 && TargetTriple.getArch() == Triple::tpc) {
      // The search for the unroll metadata is awkward and not precise since
      // the algorythm deals with the highest level loop only
      for (auto &L :  CurLoop->getLoopsInPreorder()) {
        if (!L->getLoopLatch()) continue;
        MDNode* LoopMD = L->getLoopLatch()->getBasicBlock()->getTerminator()
