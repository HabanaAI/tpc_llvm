...
MachineBlockPlacement::findBestLoopTopHelper(
  ...
  for (MachineBasicBlock *Pred : OldTop->predecessors()) {
    ...
    // The layouter prefers to place loop tails above loop head because in some
    // cases it leads to less jumps. But this behavious doesn't allow us
    // to create hardware loops, we need original loop layout.
    // This change prohibits loop tails to be placed on top of a loop chain
    // as it was in previous LLVM versions
    unsigned succ_limit = 2;
    const Module *M = Pred->getParent()->getFunction().getParent();
    Triple TargetTriple(M->getTargetTriple());
    if (TargetTriple.getArch() == Triple::tpc) {
      succ_limit = 1;
    }
    if (Pred->succ_size() > succ_limit)
#else
    if (Pred->succ_size() > 2)
#endif
      continue;
