  ...
  for (const auto &GE : GlobalExprs) {
    ...
    if (Global) {
      ...
      if (Global->isThreadLocal()) {
        ...
        auto trio = Asm->TM.getTargetTriple();
        if (trio.getArch() == Triple::tpc) {
          // need to add segment
          int as = Global->getType()->getAddressSpace();
          // expr for DW_AT_segment
          DIELoc *segLoc = nullptr;
          std::unique_ptr<DIEDwarfExpression> segDwarfExpr;
          segLoc = new (DIEValueAllocator) DIELoc;
          segDwarfExpr = std::make_unique<DIEDwarfExpression>(*Asm, *this, *segLoc);
          addUInt(*segLoc, dwarf::DW_FORM_data1, dwarf::DW_OP_constu);
          addUInt(*segLoc, dwarf::DW_FORM_data1, as);
          addBlock(*VariableDIE, dwarf::DW_AT_segment, segDwarfExpr->finalize());
        } else {
#endif
...
#ifdef LLVM_TPC_COMPILER
      // Add variable address.
      auto *DIExpr = DV.getSingleExpression();
      if (DIExpr && DIExpr->getElement(0) == dwarf::DW_OP_TPC_glob_adress) {
        int as = DIExpr->getElement(2);
        // expr for DW_AT_segment
        DIELoc *segLoc = nullptr;
        std::unique_ptr<DIEDwarfExpression> segDwarfExpr;
        segLoc = new (DIEValueAllocator) DIELoc;
        segDwarfExpr =
            std::make_unique<DIEDwarfExpression>(*Asm, *this, *segLoc);
        addUInt(*segLoc, dwarf::DW_FORM_data1, dwarf::DW_OP_constu);
        addUInt(*segLoc, dwarf::DW_FORM_data1, as);
        addBlock(*VariableDie, dwarf::DW_AT_segment, segDwarfExpr->finalize());

        // expr for DW_AT_location
        DIELoc *Loc = new (DIEValueAllocator) DIELoc;
        DIEDwarfExpression DwarfExpr(*Asm, *this, *Loc);
        addUInt(*Loc, dwarf::DW_FORM_data1, dwarf::DW_OP_constu);
        addUInt(*Loc, dwarf::DW_FORM_udata, DIExpr->getElement(1));
        addBlock(*VariableDie, dwarf::DW_AT_location, DwarfExpr.finalize());
        return VariableDie;
      }
#endif
