...
SDValue DAGCombiner::visitREM(SDNode *N) {
  ...
  if (DAG.isKnownNeverZero(N1) && !TLI.isIntDivCheap(VT, Attr)) {
    ...
    if (OptimizedDiv.getNode()) {
      ...
      Triple trip = MF.getTarget().getTargetTriple();
      SDValue N1Trunc, Mul;
      if (trip.getArch() == Triple::tpc) {
        N1Trunc = DAG.getNode(ISD::TRUNCATE, DL, VT, N1);
        Mul = DAG.getNode(ISD::MUL, DL, VT, OptimizedDiv, N1Trunc);
      } else {
        Mul = DAG.getNode(ISD::MUL, DL, VT, OptimizedDiv, N1);
      }
#else
