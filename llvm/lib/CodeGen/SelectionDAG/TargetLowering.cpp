...
SDValue TargetLowering::BuildUDIV(SDNode *N, SelectionDAG &DAG,
  ...
  auto BuildUDIVPattern = [&](ConstantSDNode *C) {
    ...
    #ifdef LLVM_TPC_COMPILER
      APInt::mu magics;
      if (TargetTriple.getArch() == Triple::tpc && Divisor.getBitWidth() > 16
          && Divisor.getZExtValue() < APInt::getMaxValue(16).getZExtValue()) {
        APInt FixedDivisor(16, 0);
        FixedDivisor |= Divisor.trunc(16);
        magics = FixedDivisor.magicu();
        VT = EVT::getIntegerVT(*DAG.getContext(), 16);
      } else {
        magics = Divisor.magicu();
      }
    #else
    ...
    if (magics.a != 0 && !Divisor[0]) {
      ...
      // Get magic number for the shifted divisor.
          ...
          if (TargetTriple.getArch() == Triple::tpc && Divisor.getBitWidth() > 16
            ...
            APInt FixedDivisor(16, 0);
            FixedDivisor |= Divisor.trunc(16);
            magics = FixedDivisor.lshr(PreShift).magicu(PreShift);;
          } else {
    ...
    if (TargetTriple.getArch() == Triple::tpc && Divisor.getBitWidth() > 16
        && Divisor.getZExtValue() < APInt::getMaxValue(16).getZExtValue()) {
      MagicFactors.push_back(DAG.getConstant(Magic, dl, EVT::getIntegerVT(*DAG.getContext(), 16)));
    } else {
  ...
  // Multiply the numerator (operand 0) by the magic value.
  #ifdef LLVM_TPC_COMPILER
  if (TargetTriple.getArch() == Triple::tpc && VT == EVT::getIntegerVT(*DAG.getContext(), 16)) {
    SDValue TruncQ = SDValue(DAG.getNode(ISD::TRUNCATE, dl, VT, Q));
    Q = DAG.getNode(ISD::MULHU, dl, VT, TruncQ, MagicFactor);
    Created.push_back(TruncQ.getNode());
  } else {
  ...
  if (UseNPQ) {
    ...
    SDValue NPQ;
    if (TargetTriple.getArch() == Triple::tpc && VT == EVT::getIntegerVT(*DAG.getContext(), 16)) {
      SDValue TruncN0 = SDValue(DAG.getNode(ISD::TRUNCATE, dl, VT, N0));
      Created.push_back(TruncN0.getNode());
      NPQ = DAG.getNode(ISD::SUB, dl, VT, TruncN0, Q);
      Created.push_back(NPQ.getNode());
    } else {
      NPQ = DAG.getNode(ISD::SUB, dl, VT, N0, Q);
      Created.push_back(NPQ.getNode());
    }
    #else
  ...
  #ifdef LLVM_TPC_COMPILER
  if (TargetTriple.getArch() == Triple::tpc && VT == EVT::getIntegerVT(*DAG.getContext(), 16)) {
    SDValue Res = DAG.getNode(ISD::SRL, dl, VT, Q, PostShift);
    Created.push_back(Res.getNode());
    Q = DAG.getNode(ISD::ZERO_EXTEND, dl, N->getValueType(0), Res);
    Created.push_back(Q.getNode());
    VT = N->getValueType(0);
  } else {
