...
bool LLParser::ParseFnAttributeValuePairs(AttrBuilder &B,
  ...
  while (true) {
    ...
    case lltok::kw_IndexSpace:
      B.addAttribute(Attribute::IndexSpace);
      break;
    case lltok::kw_IndexSpaceb:
      B.addAttribute(Attribute::IndexSpaceb);
      break;
    case lltok::kw_ReductionOrNormAxes:
      B.addAttribute(Attribute::ReductionOrNormAxes);
      break;
    case lltok::kw_UnrollInfo:
      B.addAttribute(Attribute::UnrollInfo);
      break;
...
bool LLParser::ConvertValIDToValue(Type *Ty, ValID &ID, Value *&V,
  ...
  case ValID::t_APFloat:
    ...
    if (&ID.APFloatVal.getSemantics() == &APFloat::IEEEdouble()) {
      ...
      else if (Ty->isBFloat16Ty())
        ID.APFloatVal.convert(APFloat::BFloat16(), APFloat::rmNearestTiesToEven,
                              &Ignored);
      else if (Ty->isF8_143Ty())
        ID.APFloatVal.convert(APFloat::Float8_143(), APFloat::rmNearestTiesToEven,
                              &Ignored);
      else if (Ty->isF8_152Ty())
        ID.APFloatVal.convert(APFloat::Float8_152(), APFloat::rmNearestTiesToEven,
                              &Ignored);
#endif
    }
