...
Type *Type::getPrimitiveType(LLVMContext &C, TypeID IDNumber) {
  ...
  case F8_143TyID    : return getF8_143Ty(C);
  case F8_152TyID    : return getF8_152Ty(C);
#endif
  default:
    return nullptr;
  }
...
TypeSize Type::getPrimitiveSizeInBits() const {
  switch (getTypeID()) {
#ifdef LLVM_TPC_COMPILER
  case Type::F8_143TyID: return TypeSize::Fixed(8);
  case Type::F8_152TyID: return TypeSize::Fixed(8);
#endif
...
#ifdef LLVM_TPC_COMPILER
  if (getTypeID() == F8_143TyID) return 4;
  if (getTypeID() == F8_152TyID) return 3;
#endif
