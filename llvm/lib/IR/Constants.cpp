...
Constant *Constant::getNullValue(Type *Ty) {
  ...
  case Type::F8_143ID:
    return ConstantFP::get(Ty->getContext(),
                           APFloat::getZero(APFloat::Float8_143()));
  case Type::F8_152ID:
    return ConstantFP::get(Ty->getContext(),
                           APFloat::getZero(APFloat::Float8_152()));
  case Type::BFloat16ID:
    return ConstantFP::get(Ty->getContext(),
                           APFloat::getZero(APFloat::BFloat16()));
#endif
  case Type::FloatTyID:
    return ConstantFP::get(Ty->getContext(),
...
#ifdef LLVM_TPC_COMPILER
  if (Ty->isF8_143Ty())
    return &APFloat::Float8_143();
  if (Ty->isF8_152Ty())
    return &APFloat::Float8_152();
  if (Ty->isBFloat16Ty())
    return &APFloat::BFloat16();
#endif
  if (Ty->isHalfTy())
...
ConstantFP* ConstantFP::get(LLVMContext &Context, const APFloat& V) {
  ...
  if (!Slot) {
    Type *Ty;
#ifdef LLVM_TPC_COMPILER
    if (&V.getSemantics() == &APFloat::Float8_143())
      Ty = Type::getF8_143Ty(Context);
    else if (&V.getSemantics() == &APFloat::Float8_152())
      Ty = Type::getF8_152Ty(Context);
    else if (&V.getSemantics() == &APFloat::BFloat16())
      Ty = Type::getBFloat16Ty(Context);
    else
#endif
...
#ifdef LLVM_TPC_COMPILER
  case Type::F8_143ID:
    if (&Val2.getSemantics() == &APFloat::Float8_143())
      return true;
    Val2.convert(APFloat::Float8_143(), APFloat::rmNearestTiesToEven, &losesInfo);
    return !losesInfo;
  case Type::F8_152ID:
    if (&Val2.getSemantics() == &APFloat::Float8_152())
      return true;
    Val2.convert(APFloat::Float8_152(), APFloat::rmNearestTiesToEven, &losesInfo);
    return !losesInfo;
  case Type::BFloat16ID:
    if (&Val2.getSemantics() == &APFloat::BFloat16())
      return true;
    Val2.convert(APFloat::BFloat16(), APFloat::rmNearestTiesToEven, &losesInfo);
    return !losesInfo;
#endif
  case Type::FloatTyID: {
