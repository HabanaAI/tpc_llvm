...
void TypePrinting::print(Type *Ty, raw_ostream &OS) {
  ...
  case Type::BFloat16ID:    OS << "bfloat"; return;
  case Type::F8_143ID:      OS << "f8_143"; return;
  case Type::F8_152ID:      OS << "f8_152"; return;
#endif
  ...
  if (const ConstantFP *CFP = dyn_cast<ConstantFP>(CV)) {
    ...
    if (&APF.getSemantics() == &APFloat::x87DoubleExtended()) {
      ...
      Out << format_hex_no_prefix(API.getZExtValue(), 4,
                                  /*Upper=*/true);
#ifdef LLVM_TPC_COMPILER
    } else if (&CFP->getValueAPF().getSemantics() == &APFloat::Float8_143()) {
      Out << 'H';
      Out << format_hex_no_prefix(API.getZExtValue(), 4,
                                  /*Upper=*/true);
    } else if (&CFP->getValueAPF().getSemantics() == &APFloat::Float8_152()) {
      Out << 'H';
      Out << format_hex_no_prefix(API.getZExtValue(), 4,
                                  /*Upper=*/true);
    } else if (&CFP->getValueAPF().getSemantics() == &APFloat::BFloat16()) {
      Out << 'H';
      Out << format_hex_no_prefix(API.getZExtValue(), 4,
                                  /*Upper=*/true);
#endif
...
static void writeDIExpression(raw_ostream &Out, const DIExpression *N,
  ...
  if (N->isValid()) {
#ifdef LLVM_TPC_COMPILER
    /* LLVM did not implement properly support for DW_OP_LLVM_convert
     * so we should fix it for LLVM_TPC_COMPILER GAUD-1383
     * maybe issue will be fixed in next release
     */
    auto fst = N->expr_op_begin();
    if (fst.getBase() && fst->getOp() == dwarf::DW_OP_LLVM_convert) {
