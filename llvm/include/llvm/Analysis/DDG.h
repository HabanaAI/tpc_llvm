...
/// Subclass of DDGNode representing the exit node of the graph.
/// There should only be one such node in a given graph.
class ExitDDGNode : public DDGNode {
public:
  ExitDDGNode() : DDGNode(NodeKind::Exit) {}
  ExitDDGNode(const ExitDDGNode &N) = delete;
  ExitDDGNode(ExitDDGNode &&N) : DDGNode(std::move(N)) {}
  ~ExitDDGNode() {}

  /// Define classof to be able to use isa<>, cast<>, dyn_cast<>, etc.
  static bool classof(const DDGNode *N) {
    return N->getKind() == NodeKind::Exit;
  }
  static bool classof(const ExitDDGNode *N) { return true; }
};
...
public:
  ...
  enum class EdgeKind {
    ...
    Rooted,
    Exiting,
    Last = Exiting // Must be equal to the largest enum value.
  };
...
public:
  ...
  DependenceGraphInfo(const std::string &N, const DependenceInfo &DepInfo)
      : Name(N), DI(DepInfo), Root(nullptr), Exit(nullptr) {}
  ...
  NodeType &getExit() const {
    assert(Exit && "Exit node is not available yet. Graph construction may "
                   "still be in progress\n");
    return *Exit;
  }
...
protected:
  ...
  // A special node where exits converges.
  NodeType *Exit = nullptr;
};
...
public:
  ...
  DDGEdge &createUseDefEdge(DDGNode &Src, DDGNode &Tgt) final override {
    auto *E = new DDGEdge(Tgt, DDGEdge::EdgeKind::RegisterUseDef);
    assert(E && "Failed to allocate memory for edge");
    Graph.connect(Src, Tgt, *E);
    return *E;
  }
  ...
  DDGEdge &createExitedEdge(DDGNode &Src, DDGNode &Tgt) final override {
    ...
    assert(E && "Failed to allocate memory for edge");
    assert(isa<ExitDDGNode>(Tgt) && "Expected exit node");
    Graph.connect(Src, Tgt, *E);
    return *E;
  }
