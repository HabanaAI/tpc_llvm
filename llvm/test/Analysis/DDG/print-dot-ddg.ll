; RUN: opt -aa-pipeline=basic-aa -ddg-simplify -ddg-pi-blocks -has-usedef-edge=false -passes=dot-ddg -dot-ddg-filename-prefix=%t < %s 2>&1 > /dev/null
; RUN: FileCheck %s -input-file=%t.foo.for.body.dot
; RUN: opt -aa-pipeline=basic-aa -ddg-simplify -ddg-pi-blocks -has-usedef-edge=false -passes=dot-ddg -dot-ddg-filename-prefix=%t -dot-ddg-only < %s 2>&1 > /dev/null
