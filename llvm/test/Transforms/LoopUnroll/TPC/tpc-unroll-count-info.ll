; RUN: opt --loop-unroll -tpc-unroll-count=4 -mtriple=tpc -mcpu=gaudi -S %s | FileCheck %s
; CHECK: define {{.*}}void @main{{.*}} !unroll_info [[METADATA:.*]] {
; CHECK: [[METADATA]] = !{!"{4, 4}"}
...
for.body:                                         ; preds = %for.body.lr.ph, %for.cond.cleanup17
...
for.body18:                                       ; preds = %for.body18.preheader, %for.cond.cleanup22
...
for.cond.cleanup22:                               ; preds = %for.cond.cleanup22.loopexit, %for.body18
  ...
  br i1 %exitcond126, label %for.cond.cleanup17.loopexit, label %for.body18, !llvm.loop !5

for.body23:                                       ; preds = %for.body23.preheader, %for.cond.cleanup27
...
for.cond.cleanup27:                               ; preds = %for.cond.cleanup27.loopexit, %for.body23
  ...
  br i1 %exitcond125, label %for.cond.cleanup22.loopexit, label %for.body23, !llvm.loop !6

for.body28:                                       ; preds = %for.body28.preheader, %for.cond.cleanup32
...
for.cond.cleanup32.loopexit:                      ; preds = %for.body33
  %vecins34.lcssa = phi <5 x i32> [ %vecins34, %for.body33 ]
...
for.cond.cleanup32:                               ; preds = %for.cond.cleanup32.loopexit, %for.body28
  ...
  br i1 %exitcond124, label %for.cond.cleanup27.loopexit, label %for.body28, !llvm.loop !7

for.body33:                                       ; preds = %for.body33.preheader, %for.body33
  %w.0108 = phi i32 [ %add35, %for.body33 ], [ %mul4, %for.body33.preheader ]
  %ifmCoords.4107 = phi <5 x i32> [ %vecins34, %for.body33 ], [ %vecins29, %for.body33.preheader ]
  ...
  %exitcond = icmp eq i32 %add35, %mul6
  br i1 %exitcond, label %for.cond.cleanup32.loopexit, label %for.body33, !llvm.loop !8
}
