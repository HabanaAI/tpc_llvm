...
entry:
  ...
  %dval = call <256 x float> @llvm.tpc.convert.v256f32.v256i8.v256i1(<256 x i8> %srcval, i8 4, i32 0, <256 x float> undef, <256 x i1> %vpred, i1 1)
  store <256 x float> %dval, <256 x float> addrspace(2)* %dptr, align 256
  ret void
}

declare <256 x float> @llvm.tpc.convert.v256f32.v256i8.v256i1(<256 x i8>, i8, i32, <256 x float>, <256 x i1>, i1)

; CHECK-DAG: ld_l_v     [[VRF:%V[0-9]+]], %S1
; CHECK-DAG: ld_l_v     [[VPRF:%VP[0-9]+]], %S2
; CHECK:     convert.i8 all_lanes target_type=fp32 rhne %A[[ARF:[0-9]+]], [[VRF]], ![[VPRF]]
