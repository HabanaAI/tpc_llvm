...
entry:
  ...
  %dval = call <256 x float> @llvm.tpc.convert.v256f32.v256i32.i1(<256 x i32> %srcval, i8 2, i32 0, <256 x float> %inval, i1 %spred, i1 0)
  store <256 x float> %dval, <256 x float> addrspace(2)* %dptr, align 256
  ret void
}

declare <256 x float> @llvm.tpc.convert.v256f32.v256i32.i1(<256 x i32>, i8, i32, <256 x float>, i1, i1)

; CHECK-DAG: cmp_neq.i32 [[SPRF:%SP[0-9]+]], %S3, 0x0
; CHECK-DAG: ld_l_v      %V[[ARFs:[0-9]+]], %S1
; CHECK-DAG: ld_l_v      %V[[ARFd:[0-9]+]], %S2
; CHECK:     convert.i32  all_lanes target_type=fp32 rhne %A[[ARFd]],  %A[[ARFs]], [[SPRF]]
; CHECK-DAG: st_l_v      %S0, %V[[ARFd]]
; CHECK-DAG: st_l_v
; CHECK-DAG: st_l_v
; CHECK-DAG: st_l_v
