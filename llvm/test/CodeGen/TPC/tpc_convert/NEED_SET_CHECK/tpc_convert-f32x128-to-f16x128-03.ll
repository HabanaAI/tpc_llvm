...
entry:
  ...
  %dptr = inttoptr i32 %dest to <128 x half> addrspace(2)*
  %pptr = inttoptr i32 %pred to <256 x i1> addrspace(2)*
  %iptr = inttoptr i32 %in to <128 x half> addrspace(2)*
  %srcval = load <128 x float>, <128 x float> addrspace(2)* %sptr, align 256
  %vpred = load <256 x i1>, <256 x i1> addrspace(2)* %pptr, align 256
  %inval = load <128 x half>, <128 x half> addrspace(2)* %iptr, align 256
  %dval = call <128 x half> @llvm.tpc.convert.v128f16.v128f32.v256i1(<128 x float> %srcval, i8 0, i32 2816, <128 x half> %inval, <256 x i1> %vpred, i1 0)
  store <128 x half> %dval, <128 x half> addrspace(2)* %dptr, align 256
  ret void
}

declare <128 x half> @llvm.tpc.convert.v128f16.v128f32.v256i1(<128 x float>, i8, i32, <128 x half>, <256 x i1>, i1)
