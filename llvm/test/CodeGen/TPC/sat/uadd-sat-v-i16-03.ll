...
entry:
  ...
  %splat.0 = insertelement <128 x i16> undef, i16 777, i32 0
  %x2 = shufflevector <128 x i16> %splat.0, <128 x i16> undef, <128 x i32> zeroinitializer
  %add = call <128 x i16> @llvm.uadd.sat.v64i32(<128 x i16> %x1, <128 x i16> %x2)
  store <128 x i16> %add, <128 x i16> addrspace(2)* %dptr
  ret void
}

declare <128 x i16> @llvm.uadd.sat.v64i32(<128 x i16>, <128 x i16>)

; CHECK: add.u16 st [[DEST:%V[0-9]+]], {{%V[0-9]+}}, 0x309
