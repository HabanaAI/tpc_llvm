...
entry:
  ...
  %4 = inttoptr i32 %val2 to <64 x i32> addrspace(2)*

  %s1 = load <64 x i32>, <64 x i32> addrspace(2)* %1, align 256
  %s2 = load <64 x i32>, <64 x i32> addrspace(2)* %2, align 256
  %v1 = load <64 x i32>, <64 x i32> addrspace(2)* %3, align 256
  %v2 = load <64 x i32>, <64 x i32> addrspace(2)* %4, align 256

  %mask = icmp sgt <64 x i32> %v1, %v2

  %vr = select <64 x i1> %mask, <64 x i32> zeroinitializer, <64 x i32> %s1
...
; CHECK:     sel_less.i32 [[VAL:%V[0-9]+]], [[VAL1]], [[VAL2]], [[SRC1]], 0x0
; CHECK:     st_l_v       %S0{{.*}}, [[VAL]]
