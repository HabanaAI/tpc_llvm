...
define void @main(i32 %dest, i32 %src1, i32 %src2, i32 %pred, float %val) {
entry:
  ...
  %splat.splatinsert = insertelement <64 x float> undef, float %val, i32 0
  %valv = shufflevector <64 x float> %splat.splatinsert, <64 x float> undef, <64 x i32> zeroinitializer

  %mask = fcmp ogt <64 x float> %vp, %valv
...
; CHECK:     sel_grt.f32 [[VAL:%V[0-9]+]], [[PRED]], %S4, [[VAL1]], [[VAL2]]
; CHECK:     st_l_v      %S0{{.*}}, [[VAL]]
