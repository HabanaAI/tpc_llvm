...
def parse_args():
    ...
    execution_group.add_argument("--no-indirectly-run-check",
            ...
            help="Do not error if a test would not be run if the user had "
                 ...
                 "test directly.",
            action="store_false")
    execution_group.add_argument("--no-execute-xfail",
            dest="skipXFAIL",
            help="Don't execute XFAIL tests",
            action="store_true")
...
# LLVM_TPC_COMPILER
    selection_group.add_argument("--nfilter",
            metavar="REGEX",
            type=_case_insensitive_regex,
            help=("Do not run tests with paths matching the given "
                  "regular expression"),
            default=os.environ.get("LIT_NFILTER"))
    selection_group.add_argument("--nightly",
            help="Enable tests of nightly testing",
            action="store_true",
            default=False)
# /LLVM_TPC_COMPILER
