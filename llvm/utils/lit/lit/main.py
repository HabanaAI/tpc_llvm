...
# LLVM_TPC_COMPILER
    if opts.nfilter:
        filtered_tests = [t for t in selected_tests if
                          not opts.nfilter.search(t.getFullName())]
        if not filtered_tests:
            sys.stderr.write('error: negative filter excluded all tests '
                             '(of %d filtered).  ' % len(filtered_tests))
            if opts.allow_empty_runs:
