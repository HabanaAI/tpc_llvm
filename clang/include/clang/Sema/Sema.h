...
public:
  ...
  enum class PragmaTpcPrintfAction {
    Enable,
    Disable
  };

  /// True if printf is enabled.
  bool TpcPrintfPragmaIsOn = false;

  // True if printf was used in the translation unit.
  bool TpcPrintfIsUsed = false;

  /// Declarations of constants, corresponding to the used tensors.
  SmallVector<VarDecl *, 16> Tensors;

  /// Declarations of main function parameters passed in special tensor.
  SmallVector<ParmVarDecl*, 4> ExtraParams;

  /// Called on well formed \#pragma tpc_printf (enable/disable).
  void ActOnPragmaTpcPrintf(PragmaTpcPrintfAction Action);

  bool isPrintfEnabled(FunctionDecl *FDecl);

  bool checkBeforeCodegen(Decl *D);
  void checkTranslationUnin();
  VarDecl *ActOnTensorDeclarator(VarDecl *TensorDecl, unsigned TensorNumber,
                                 bool ExtraArgsPresent);
  DeclStmt *ProcessExtraArgs(ParmVarDecl* Arg, VarDecl* Ptr);
  Stmt *ShiftIndex(VarDecl* TensorOffset);
  Stmt *getArgumentAddress(VarDecl* Ptr, VarDecl* TensorOffset);
  VarDecl *CreateTensorOffsetForExtraArgs();
  VarDecl *CreateGlobPointer();
  void DeclareExtraArgs(SmallVector<Stmt*, 32> &Stmts,
                        const SmallVector<ParmVarDecl*, 4> &ExtraArgs);
  void loadRC99Types();
  bool CheckNotIndexType(Expr *E, SourceLocation Loc);
  Expr *buildDefaultArgument(const FunctionProtoType *FT,
                             unsigned ID, unsigned ParamNo);

  // Action function for processing index space arguments.
  void ActOnPragmaIndexSpace(
  ...
  // Action function for processing index space b arguments.
  void ActOnPragmaIndexSpaceb(
      Scope *curScope, SourceLocation PragmaLoc,
      llvm::SmallVector<unsigned, 16> &TensorIDs,
      llvm::SmallVector<llvm::SmallString<128>, 10> &IndexFactors);

  // Action function for processing reduction/normalization axes arguments.
  void ActOnPragmaReductionOrNormAxes(
      Scope *curScope, SourceLocation PragmaLoc,
      llvm::SmallVector<unsigned, 16> &TensorIDs,
