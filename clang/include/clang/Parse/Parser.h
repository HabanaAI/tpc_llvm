...
class Parser : public CodeCompletionHandler {
  ...
  std::unique_ptr<PragmaHandler> Tpc_printfHandler;
  std::unique_ptr<PragmaHandler> LoopUnrollHintHandler;
  std::unique_ptr<PragmaHandler> LoopUnrollDivideHintHandler;
  std::unique_ptr<PragmaHandler> LoopTakenHintHandler;
  std::unique_ptr<PragmaHandler> LoopPipelineHintHandler;
  std::unique_ptr<PragmaHandler> IndexSpaceHandler;
  std::unique_ptr<PragmaHandler> IndexSpacebHandler;
  std::unique_ptr<PragmaHandler> ReductionAxesHandler;
  std::unique_ptr<PragmaHandler> NormAxesHandler;
#endif
...
private:
  ...
  void HandlePragmaTpc_printf();
  /// Handle the annotation tokens produced
  /// for index space ie <tensor_vector>,
  /// <stride_vector>
  void HandlePragmaIndexSpace();
  /// Handle the annotation tokens produced
  /// for index space b ie <tensor_vector>,
  /// <stride_vector>
  void HandlePragmaIndexSpaceb();
  /// Handle the annotation tokens produced
  /// for reduction/norm axes ie <axis_vector>
  void HandlePragmaReductionOrNormAxes();
#endif
...
#ifdef LLVM_TPC_COMPILER
  /// Main function parameters of tensor type.
  SmallVector<VarDecl*, 4> Tensors;

  /// Main function parameters passed in special tensor.
  SmallVector<ParmVarDecl*, 4> ExtraArgs;

  static const unsigned int MaxMainArgs = 32;
#endif
