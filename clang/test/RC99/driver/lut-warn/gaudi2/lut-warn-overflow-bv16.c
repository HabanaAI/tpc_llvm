// RUN: %tpc_clang %s -S -march=gaudi2 2>&1 | FileCheck %s
// XFAIL: *
void main(int x0, int x2, int x3, int dest0, int dest1, int dest2)
{
 ...
 float64 __local *res2 = (float64  __local *)dest2;
  ...
  temp_res0 = v_f32_lookup_1c(*ptr_x0, e_u16_sqrt, 1, temp_res0, x3, 0);
  temp_res1 = v_f32_lookup_1c(*ptr_x0, e_u16_sqrt, 1, temp_res0, x3, 0);
  temp_res2 = v_f32_lookup_1c(*ptr_x0, e_bf16_rcp_scalar_m7, 1, temp_res0, x3, 0);
  temp_res3 = v_f32_lookup_1c(*ptr_x0, e_bf16_sin_cos_poly2_m4_c0, 1, temp_res0, x3, 0);
