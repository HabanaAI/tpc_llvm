...
void main(int dest, float value, int start, int end, int stride) {
  ...
  #pragma clang loop machine_unroll_count(4) divide(enable) taken(enable)
...
// CHECK-NEXT: LoopHintAttr {{.*}} loop MachineUnrollCount Numeric
...
// LLVMIR: [[LOOP]] = distinct !{[[LOOP]], [[COUNT:![0-9]+]], [[DIVIDE:![0-9]+]], [[TAKEN:![0-9]+]]}
