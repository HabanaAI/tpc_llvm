// RUN: %clang_cc1 -triple tpc-none-none -std=rc99 -target-cpu gaudi -bfloat16 %s -verify
...
void main(signed char x0, _Bool pred) {
  ...
  ires = s_i8_mac(x0, 1, ires, 0, 1, 0);            // todo:
  ires = s_u8_mac(x0, 1, ires, 0, 1, 0);            // todo:
  ires = s_i16_mac(x0, 1, ires, SW_NEG, 1, 0);      // todo:
}
