...
void main(unsigned addr, int dest) {
  ...
  _BFloat16 result = s_bf16_ld_l(addr, 0, 0, 1, 0);
  *dptr++ = result;
  
  result = s_bf16_ld_l(addr, 1, 0, 1, 0);
  *dptr++ = result;
  
  result = s_bf16_ld_l(0x20, 0, 0, 1, 0);
  *dptr++ = result;
  
  result = s_bf16_ld_l(0x20, 1, 0, 1, 0);
  *dptr = result;
}
...
//CHECK: ld_l     %S[[Val3:[0-9]+]], 0x20
//CHECK: st_l     %S{{[0-9]+}}, %S[[Val3]]

//CHECK: ld_l     mmio %S[[Val4:[0-9]+]], 0x20
