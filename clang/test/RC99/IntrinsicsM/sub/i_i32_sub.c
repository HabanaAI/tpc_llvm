...
void main(int x0, int x1, int dest, _Bool pred) {
  ...
  *dptr++ = res;
  // CHECK: sub.i32  b10000 %I{{[0-9]+}}, %I{{[0-9]+}}, %I{{[0-9]+}}, %SP{{[0-9]+}}

  res = i_i32_sub(a, b, 0x08, 0, res, pred, 1);
  *dptr++ = res;
  // CHECK: sub.i32 b01000 %I{{[0-9]+}}, %I{{[0-9]+}}, %I{{[0-9]+}}, !%SP{{[0-9]+}}
  ...
  *dptr++ = res;
  // CHECK: sub.i32  b10000 %I{{[0-9]+}}, %S1, %I{{[0-9]+}}, %SP{{[0-9]+}}
  ...
  *dptr++ = res;
  // CHECK: sub.i32 b00010 %I{{[0-9]+}}, 0x7b, %I{{[0-9]+}}, %SP{{[0-9]+}}

  res = i_i32_sub(123, a, 0x04, 0, res, pred, 1);
  *dptr++ = res;
  // CHECK: sub.i32 b00100 %I{{[0-9]+}}, 0x7b, %I{{[0-9]+}}, !%SP{{[0-9]+}}
}
