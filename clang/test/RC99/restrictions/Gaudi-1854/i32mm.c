...
void main(tensor data, tensor scale, tensor pred, tensor out)
{
    ...
    int128 res1 = {0};
    res1  = v_i32_mul_vb(input1, 1, SW_KEEP_RS, res1, vpred1);

    bool64 vpred2 = to_bool64(v_i1_ld_tnsr_b(pred_index, pred));
    int64 res2 = 0;
    res2  = v_i32_ash_rhaz_vb(res1, input2, 0, res2, vpred2);
...
// CHECK-DAG: mul.i32  keep_rs_for_add {{%D[0-9]+}}, {{%V[0-9]+}}, 0x1, {{%VP[0-9]+}}
// CHECK-DAG: add.i32  [[DEST:%V[0-9]+]], {{%V[0-9]+}}, {{%V[0-9]+}}
// CHECK-DAG: and.i32  [[SHIFT:%V[0-9]+]], [[ORIG]], 0xff
// CHECK-DAG: sel_eq.i32 {{%V[0-9]+}}, [[SHIFT]], 0x0, [[DEST]], {{%V[0-9]+}}, {{%VP[0-9]+}}
