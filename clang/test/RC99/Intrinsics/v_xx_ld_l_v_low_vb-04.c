...
void main(unsigned addr, int dest, int vpredp) {
  ...
  
  {
    ...
    result = v_f8_ld_l_v_low_vb(addr, 0, result, vpred, 0);
    // CHECK: ld_l_v_low  %V{{[0-9]+}}, %S0, %VP{{[0-9]+}}
    *dptr++ = result;
    
    result = v_f8_ld_l_v_low_vb(0x100, 0, result, vpred, 0);
    // CHECK: ld_l_v_low  %V{{[0-9]+}}, 0x100, %VP{{[0-9]+}}
    *dptr++ = result;
  }

  {
    ...
    result = v_h8_ld_l_v_low_vb(addr, 0, result, vpred, 0);
    // CHECK: ld_l_v_low  %V{{[0-9]+}}, %S0, %VP{{[0-9]+}}
    *dptr++ = result;
    
    result = v_h8_ld_l_v_low_vb(0x100, 0, result, vpred, 0);
    // CHECK: ld_l_v_low  %V{{[0-9]+}}, 0x100, %VP{{[0-9]+}}
    *dptr++ = result;
  }
}
