...
void main(unsigned addr, int dest, _Bool pred) {
  ...
  
  {
    ...
    result = v_f8_ld_l_v_low_b(addr, 0, result, pred, 0);
    // CHECK: ld_l_v_low  %V{{[0-9]+}}, %S0, %SP{{[0-9]+}}
    *dptr++ = result;
    
    result = v_f8_ld_l_v_low_b(0x100, 0, result, pred, 0);
    // CHECK: ld_l_v_low  %V{{[0-9]+}}, 0x100, %SP{{[0-9]+}}
    *dptr++ = result;
  }

  {
    ...
    result = v_h8_ld_l_v_low_b(addr, 0, result, pred, 0);
    // CHECK: ld_l_v_low  %V{{[0-9]+}}, %S0, %SP{{[0-9]+}}
    *dptr++ = result;
    
    result = v_h8_ld_l_v_low_b(0x100, 0, result, pred, 0);
    // CHECK: ld_l_v_low  %V{{[0-9]+}}, 0x100, %SP{{[0-9]+}}
    *dptr++ = result;
  }
}
