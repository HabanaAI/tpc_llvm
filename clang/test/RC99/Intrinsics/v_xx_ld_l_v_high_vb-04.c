...
void main(unsigned addr, int dest, int vpredp) {
  ...
  
  {
    ...
    result = v_f8_ld_l_v_high_vb(addr, 0, result, vpred, 0);
    // CHECK: ld_l_v_high  %V{{[0-9]+}}, %S0, %VP{{[0-9]+}}
    *dptr++ = result;
    
    result = v_f8_ld_l_v_high_vb(0x100, 0, result, vpred, 0);
    // CHECK: ld_l_v_high  %V{{[0-9]+}}, 0x100, %VP{{[0-9]+}}
    *dptr++ = result;
  }

  {
    ...
    result = v_h8_ld_l_v_high_vb(addr, 0, result, vpred, 0);
    // CHECK: ld_l_v_high  %V{{[0-9]+}}, %S0, %VP{{[0-9]+}}
    *dptr++ = result;
    
    result = v_h8_ld_l_v_high_vb(0x100, 0, result, vpred, 0);
    // CHECK: ld_l_v_high  %V{{[0-9]+}}, 0x100, %VP{{[0-9]+}}
    *dptr++ = result;
  }
}
