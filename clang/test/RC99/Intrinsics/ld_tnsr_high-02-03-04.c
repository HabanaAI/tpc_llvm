...
void main(tensor input, int dest, _Bool pred, int vpredp) {
...

  {
    ...
    res = v_f32_ld_tnsr_high_vb(index, input, 0, res, vpred64, 0);
    *dest_ptr++ = res;
  ...
  }
  {
    ...
    res = v_bf16_ld_tnsr_high_b(index, input, 0, res, pred, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
  ...
  }
  {
    ...
    res = v_bf16_ld_tnsr_high_vb(index, input + 1, 0, res, vpred128, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_high %V{{[0-9]+}}, 0x1, %I{{[0-9]+}}, %VP{{[0-9]+}}
  }
  {
    int64 __local *dest_ptr = (int64 __local *)vector_ptr;
    int64 res = 0;

    res = v_i32_ld_tnsr_high_vb(index, input, 0, res, vpred64, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_high %V{{[0-9]+}}, 0x0, %I{{[0-9]+}}, %VP{{[0-9]+}}
  }
  {
    ...
    res = v_u32_ld_tnsr_high_vb(index, input + 1, 0, res, vpred64, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_high %V{{[0-9]+}}, 0x1, %I{{[0-9]+}}, %VP{{[0-9]+}}
  }
  {
    ...
    res = v_i16_ld_tnsr_high_vb(index, input, 0, res, vpred128, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_high %V{{[0-9]+}}, 0x0, %I{{[0-9]+}}, %VP{{[0-9]+}}
  }
  {
    ...
    res = v_u16_ld_tnsr_high_vb(index, input + 1, 0, res, vpred128, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_high %V{{[0-9]+}}, 0x1, %I{{[0-9]+}}, %VP{{[0-9]+}}
  }
  {
    ...
    res = v_i8_ld_tnsr_high_vb(index, input, 0, res, vpred256, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_high %V{{[0-9]+}}, 0x0, %I{{[0-9]+}}, %VP{{[0-9]+}}
  }
  {
    ...
    res = v_u8_ld_tnsr_high_vb(index, input + 1, 0, res, vpred256, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_high %V{{[0-9]+}}, 0x1, %I{{[0-9]+}}, %VP{{[0-9]+}}
  }
  {
    ...
    res = v_i1_ld_tnsr_high_vb(index, input, 0, res, vpred256, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_high %VP{{[0-9]+}}, 0x0, %I{{[0-9]+}}, %VP{{[0-9]+}}
  }
}
