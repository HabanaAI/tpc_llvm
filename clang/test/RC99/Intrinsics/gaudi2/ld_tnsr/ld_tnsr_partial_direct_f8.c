...
void main(unsigned addr1, unsigned addr2, int dest, _Bool pred, int vpredp,char sz,char off) {
...

  {
    ...
    res = v_f8_ld_tnsr_partial_b(addr1, addr2, sz,off,0, res, pred, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr partial %V{{[0-9]+}}, %LD_TNSR_ID_REG, %I{{[0-9]+}}, %LD_PARTIAL_REG, %SP{{[0-9]+}}
  }
  {
    minifloat256 __local *dest_ptr = (minifloat256 __local *)vector_ptr;
    minifloat256 res = 0;
    
    res = v_f8_ld_tnsr_partial_vb(addr1, addr2, sz,off,0, res, vpred256, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr partial %V{{[0-9]+}}, %LD_TNSR_ID_REG, %I{{[0-9]+}}, %LD_PARTIAL_REG, %VP{{[0-9]+}}
  }
}