...
void main(tensor input, int dest, _Bool pred, int vpredp,
  ...
  bool256 vpred256 = *vpred_ptr256++;
  {
    ...
    res = v_f8_ld_tnsr_low_partial_b(index, input, 16,6,0, res, pred, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_low partial %V{{[0-9]+}}, 0x0, %I{{[0-9]+}}, %LD_PARTIAL_REG, %SP{{[0-9]+}}
  }
  {
    minifloat256 __local *dest_ptr = (minifloat256 __local *)vector_ptr;
    minifloat256 res = 0;
    
    res = v_f8_ld_tnsr_low_partial_vb(index, input + 1,16,6, 0, 0, vpred256, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_low partial %V{{[0-9]+}}, 0x1, %I{{[0-9]+}}, %LD_PARTIAL_REG, %VP{{[0-9]+}}
  }
