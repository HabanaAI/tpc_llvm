...
void main(tensor input, int dest, _Bool pred, int vpredp,
  ...
  bool256 vpred256 = *vpred_ptr256++;
  {
    ...
    res = v_h8_ld_tnsr_high_partial_b(index, input, 16,6,0, res, pred, 0);
    *dest_ptr++ = res;
  ...
  }
  {
    ...
    res = v_h8_ld_tnsr_high_partial_vb(index, input + 1,16,6, 0, 0, vpred256, 0);
    *dest_ptr++ = res;
    vector_ptr = (int64 __local *)dest_ptr;
    // CHECK-DAG: ld_tnsr_high partial %V{{[0-9]+}}, 0x1, %I{{[0-9]+}}, %LD_PARTIAL_REG, %VP{{[0-9]+}}
  }
