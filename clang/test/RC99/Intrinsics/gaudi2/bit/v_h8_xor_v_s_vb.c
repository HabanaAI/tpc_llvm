...
void main(int x0, minihalf x1, int dest0, int dest1)
{
    ...
    temp_res0 = v_h8_xor_vb(*ptr_x0, (minihalf)8, 0,temp_res0, pred2, 0);
    *res0 = temp_res0;
    
    minihalf256 __local *res1 = (minihalf256  __local *)dest1;
    minihalf256 temp_res1 = 0;
    temp_res1 = v_h8_xor_vb(*ptr_x0, x1, 0,temp_res1, pred2, 0);
    *res1 = temp_res1;
}
//CHECK-ASM: .globl main
//CHECK-ASM-DAG: xor.f8_152 %V{{[0-9]+}}, %V{{[0-9]+}}, 0x48, %VP{{[0-9]+}}
//CHECK-ASM-DAG: xor.f8_152 %V{{[0-9]+}}, %V{{[0-9]+}}, %S{{[0-9]+}}, %VP{{[0-9]+}}
