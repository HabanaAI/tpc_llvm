...
void main(minifloat x0, minifloat x1, int x2, int dest0, int dest1)
{
    ...
    temp_res0 = s_f8_xor(x0, x1, 0,temp_res0, x2, 0);
    *res0 = temp_res0;
    
    minifloat __local *res1 = (minifloat  __local *)dest1;
    minifloat temp_res1 = 0;
    temp_res1 = s_f8_xor(x0, (minifloat)8, 0,temp_res1, x2, 0);
    *res1 = temp_res1;
}
//CHECK-ASM: .globl main
//CHECK-ASM-DAG: xor.f8_143 %S{{[0-9]+}}, %S{{[0-9]+}}, %S{{[0-9]+}}, %SP{{[0-9]+}}
//CHECK-ASM-DAG: xor.f8_143 %S{{[0-9]+}}, %S{{[0-9]+}}, 0x50, %SP{{[0-9]+}}
