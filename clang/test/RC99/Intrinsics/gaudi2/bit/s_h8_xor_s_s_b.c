...
void main(minihalf x0, minihalf x1, int x2, int dest0, int dest1)
{
    ...
    temp_res0 = s_h8_xor(x0, x1, 0,temp_res0, x2, 0);
    *res0 = temp_res0;
    
    minihalf __local *res1 = (minihalf  __local *)dest1;
    minihalf temp_res1 = 0;
    temp_res1 = s_h8_xor(x0, (minihalf)8, 0,temp_res1, x2, 0);
    *res1 = temp_res1;
}
//CHECK-ASM: .globl main
//CHECK-ASM-DAG: xor.f8_152 %S{{[0-9]+}}, %S{{[0-9]+}}, %S{{[0-9]+}}, %SP{{[0-9]+}}
//CHECK-ASM-DAG: xor.f8_152 %S{{[0-9]+}}, %S{{[0-9]+}}, 0x48, %SP{{[0-9]+}}
