...
void main(tensor in,tensor out,bool pred)
{
    ...
    d0 = v_h8_cmp_leq_b(v0,v1, 0, d0, pred, 0);
    v_i1_st_tnsr(c0,out,d0,0,1,0);
    d1 = v_h8_cmp_leq_b(v0,(minihalf)8, 0, d0, pred, 0);
    d1 = v_h8_cmp_leq_vb(v0,v1, 0, d0, d1, 0); 
    v_i1_st_tnsr(c1,out,d1,0,1,0);
}
//CHECK-ASM: .globl main
//CHECK-ASM-DAG: cmp_leq.f8_152 %VP{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}, %SP
//CHECK-ASM-DAG: cmp_leq.f8_152 %VP{{[0-9]+}}, %V{{[0-9]+}}, 0x48, %SP
//CHECK-ASM-DAG: cmp_leq.f8_152 %VP{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}, %VP
