...
void main(minihalf x0, int x1, int x2, int x4, int dest0, int dest1)
{
   ...
   minihalf256 __local *res0 = (minihalf256  __local *)dest0;
    ...
    temp_res0 = v_h8_form_fp_num_b((minihalf)8, *ptr_x1, *ptr_x2, SW_ADD_BIAS, temp_res0, x4, 0);
    *res0 = temp_res0;
    
   minihalf256 __local *res1 = (minihalf256  __local *)dest1;
    minihalf256 temp_res1 = 0;
    temp_res1 = v_h8_form_fp_num_b(x0, *ptr_x1, *ptr_x2, SW_ADD_BIAS, temp_res1, x4, 0);
    *res1 = temp_res1;
}
//CHECK-ASM: .globl main
//CHECK-ASM-DAG: form_fp_num.f8_152 exp_add_bias %V{{[0-9]+}}, 0x48, %V{{[0-9]+}}, %V{{[0-9]+}}, %SP{{[0-9]+}}
//CHECK-ASM-DAG: form_fp_num.f8_152 exp_add_bias %V{{[0-9]+}}, %S{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}, %SP{{[0-9]+}}
