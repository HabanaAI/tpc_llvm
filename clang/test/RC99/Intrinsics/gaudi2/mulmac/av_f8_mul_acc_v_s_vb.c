...
void main(int x0, minifloat x1, int dest0, int dest1)
{
   ...
   float64 __local *res0 = (float64  __local *)dest0;
    ...
    temp_res0 = v_f8_mul_acc32_vb(*ptr_x0, (minifloat)8, 0, temp_res0, pred2, 0);
    *res0 = temp_res0.v1;
    
   float64 __local *res1 = (float64  __local *)dest1;
    float256 temp_res1 = {0.f, 0.f};
    temp_res1 = v_f8_mul_acc32_vb(*ptr_x0, x1, 0, temp_res1, pred2, 0);
    *res1 = temp_res1.v1;
}
//CHECK-ASM: .globl main
//CHECK-ASM-DAG: mul.f8_143 acc_fp32 %A{{[0-9]+}}, %V{{[0-9]+}}, 0x50, %VP{{[0-9]+}}
//CHECK-ASM-DAG: mul.f8_143 acc_fp32 %A{{[0-9]+}}, %V{{[0-9]+}}, %S{{[0-9]+}}, %VP{{[0-9]+}}
