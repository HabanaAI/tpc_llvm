...
void main(int dest, float x, _Bool pred) {
  ...
  res = s_convert_f32_to_i8(x, SW_RHNE, res, pred, 0);
  *dest_ptr++ = res;
  // CHECK: convert.f32 target_type=int8 rhne [[DEST]], %S1, [[PRED]]
  
  res = s_convert_f32_to_i8(x, SW_RZ, res, pred, 0);
  *dest_ptr++ = res;
  // CHECK: convert.f32 target_type=int8 rz [[DEST]], %S1, [[PRED]]
}
