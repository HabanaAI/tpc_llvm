...
main(int s0, tensor out)
{
  int st;
  int5 cq = {0,0,0,0,0};
  bf16 f = s_convert_i32_to_bf16(s0,SW_CLIP_FP,0.0, 1,0);
  st = s_convert_i64_to_i32(-1,s0,0,0,1,0);
  s_i32_st_g(gen_addr(cq, out, 0, 0, 1, 0), st, 0, 1, 0);
  cq[0]++;
  s_bf16_st_g(gen_addr(cq, out, 0, 0, 1, 0), f, 0, 1, 0);

}
// DIS:  convert.i32 target_type=bf16 rhne clip_fp
// DIS:  convert.i64 S
