...
// CHECK-DAG: mov{{.*}}	[[PRED:%SP[0-9]+]], %S3
  ...
  // v_convert_i16_to_u16_b
  {
    ushort128 res = income;

    res = v_convert_i16_to_u16_b(x, 0, res, pred, 0);
    *dest_ptr++ = res;
// CHECK: convert.i16 target_type=uint16 rhne [[DEST]], [[SRC]], [[PRED]]

    res = v_convert_i16_to_u16_b(x, 0, res, pred, 1);
    *dest_ptr++ = res;
// CHECK: convert.i16 target_type=uint16 rhne [[DEST]], [[SRC]], ![[PRED]]

    res = v_convert_i16_to_u16_b(x, SW_RHNE, res, 1, 0);
    *dest_ptr++ = res;
// CHECK: convert.i16 target_type=uint16 rhne [[DEST]], [[SRC]], %SP0

    income = res;
  }

  // v_convert_i16_to_u16_vb
  {
    ushort128 res = income;

    res = v_convert_i16_to_u16_vb(x, 0, res, to_bool128(vpred), 0);
    *dest_ptr++ = res;
// CHECK: convert.i16 target_type=uint16 rhne [[DEST]], [[SRC]], [[VPRED]]

    res = v_convert_i16_to_u16_vb(x, 0, res, to_bool128(vpred), 1);
    *dest_ptr++ = res;
// CHECK: convert.i16 target_type=uint16 rhne [[DEST]], [[SRC]], ![[VPRED]]
