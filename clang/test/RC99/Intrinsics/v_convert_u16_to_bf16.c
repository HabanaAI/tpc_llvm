...
// CHECK-DAG: mov{{.*}}	[[PRED:%SP[0-9]+]], %S3
  ...
  // v_convert_u16_to_bf16_b
  {
    bfloat128 res = income;

    res = v_convert_u16_to_bf16_b(x, 0, res, pred, 0);
    *dest_ptr++ = res;
// CHECK: convert.u16 target_type=bf16 rhne [[DEST]], [[SRC]], [[PRED]]

    res = v_convert_u16_to_bf16_b(x, 0, res, pred, 1);
    *dest_ptr++ = res;
// CHECK: convert.u16 target_type=bf16 rhne [[DEST]], [[SRC]], ![[PRED]]

    res = v_convert_u16_to_bf16_b(x, SW_RHNE, res, 1, 0);
    *dest_ptr++ = res;
// CHECK: convert.u16 target_type=bf16 rhne [[DEST]], [[SRC]], %SP0

    income = res;
  }

  // v_convert_u16_to_bf16_vb
  {
    bfloat128 res = income;

    res = v_convert_u16_to_bf16_vb(x, 0, res, to_bool128(vpred), 0);
    *dest_ptr++ = res;
// CHECK: convert.u16 target_type=bf16 rhne [[DEST]], [[SRC]], [[VPRED]]

    res = v_convert_u16_to_bf16_vb(x, 0, res, to_bool128(vpred), 1);
    *dest_ptr++ = res;
// CHECK: convert.u16 target_type=bf16 rhne [[DEST]], [[SRC]], ![[VPRED]]
