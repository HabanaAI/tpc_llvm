...
void main(int x0a, int x1a, _BFloat16 xs, int dest, _Bool pred, int vpreda) {
  ...
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}

  res = v_bf16_mac_b(x0, x1, res, SW_NEG, 1, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}
  ...
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, %S2

  res = v_bf16_mac_b(x0, xs, res, SW_NEG, 1, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, %S2
  ...
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, 0x3fc0

  res = v_bf16_mac_b(x0, 1.5, res, SW_NEG, 1, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, 0x3fc0
  ...
  *dptr++ = res;
  ...
  res = v_bf16_mac_b(x0, x1, res, (0) << 1, 1, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}

  res = v_bf16_mac_b(x0, x1, res, (1) << 1, 1, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}

  res = v_bf16_mac_b(x0, x1, res, (0) << 1, pred, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}, %SP{{[0-9]+}}

  res = v_bf16_mac_b(x0, x1, res, (1) << 1, pred, 1);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}, !%SP{{[0-9]+}}

  res = v_bf16_mac_vb(x0, x1, res, (0) << 1, to_bool128(vpred_c), 0);
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}, %VP{{[0-9]+}}

  res = v_bf16_mac_vb(x0, x1, res, (1) << 1, to_bool128(vpred_c), 1);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, %V{{[0-9]+}}, !%VP{{[0-9]+}}

  // Vector + Scalar

  res = v_bf16_mac_b(x0, xs, res, (0) << 1, 1, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, %S2

  res = v_bf16_mac_b(x0, xs, res, (1) << 1, 1, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, %S2

  res = v_bf16_mac_b(x0, xs, res, (0) << 1, pred, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, %S2, %SP{{[0-9]+}}

  res = v_bf16_mac_b(x0, xs, res, (1) << 1, pred, 1);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, %S2, !%SP{{[0-9]+}}

  res = v_bf16_mac_vb(x0, xs, res, (0) << 1, to_bool128(vpred_c), 0);
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, %S2, %VP{{[0-9]+}}

  res = v_bf16_mac_vb(x0, xs, res, (1) << 1, to_bool128(vpred_c), 1);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, %S2, !%VP{{[0-9]+}}

  // Vector + Immediate

  res = v_bf16_mac_b(x0, 1.5, res, (0) << 1, 1, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, 0x3fc0

  res = v_bf16_mac_b(x0, 1.5, res, (1) << 1, 1, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, 0x3fc0

  res = v_bf16_mac_b(x0, 1.5, res, (0) << 1, pred, 0);
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, 0x3fc0, %SP{{[0-9]+}}

  res = v_bf16_mac_b(x0, 1.5, res, (1) << 1, pred, 1);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, 0x3fc0, !%SP{{[0-9]+}}

  res = v_bf16_mac_vb(x0, 1.5, res, (0) << 1, to_bool128(vpred_c), 0);
  *dptr++ = res;
  // CHECK: mac.bf16 %V{{[0-9]+}}, %V{{[0-9]+}}, 0x3fc0, %VP{{[0-9]+}}

  res = v_bf16_mac_vb(x0, 1.5, res, (1) << 1, to_bool128(vpred_c), 1);
  *dptr++ = res;
  // CHECK: mac.bf16 neg %V{{[0-9]+}}, %V{{[0-9]+}}, 0x3fc0, !%VP{{[0-9]+}}
}
