...
Value *ScalarExprEmitter::VisitInitListExpr(InitListExpr *E) {
  ...
  for (unsigned i = 0; i != NumInitElements; ++i) {
    ...
    if (!VVT) {
      ...
      if (CGF.getTarget().getTriple().getArch() == llvm::Triple::tpc &&
          VType->getElementType()->isIntegerTy() &&
          cast<llvm::IntegerType>(cast<llvm::FixedVectorType>(VType)->getElementType())->getBitWidth() == 1 &&
          ResElts == 256) {
        // bool256 is represented as <32 x i32> in frontend so its initializer has type
        // i32. Convert it to i1.
        if (const auto *IniVal = dyn_cast<llvm::ConstantInt>(Init)) {
          uint64_t IV = IniVal->getLimitedValue();
          if (IV == 0) {
            Init = llvm::ConstantInt::get(cast<llvm::FixedVectorType>(VType)->getElementType(), 0);
          } else if (IV == 0xff) {
            Init = llvm::ConstantInt::get(cast<llvm::FixedVectorType>(VType)->getElementType(), 1);
          } else {
            CGF.ErrorUnsupported(E, "initialization of bool256 with non-zero");
          }
        } else {
          Init = CGF.Builder.CreateICmp(llvm::CmpInst::ICMP_NE, Init, llvm::ConstantInt::get(CGF.Int8Ty, 0));
        }
      }
#endif
...
Value *ScalarExprEmitter::VisitCastExpr(CastExpr *CE) {
  ...
  case CK_VectorSplat: {
    ...
    if (CGF.getTarget().getTriple().getArch() == llvm::Triple::tpc &&
        DstTy->isVectorTy() &&
        cast<llvm::FixedVectorType>(DstTy)->getElementType()->isIntegerTy() &&
        cast<llvm::IntegerType>(cast<llvm::FixedVectorType>(DstTy)->getElementType())->getBitWidth() == 1 &&
        cast<llvm::FixedVectorType>(DstTy)->getNumElements() == 256) {
      // bool256 is represented as <32 x i32> in frontend so its initializer has type
      // i32. Convert it to i1.
      if (const auto *IniVal = dyn_cast<llvm::ConstantInt>(Elt)) {
        uint64_t IV = IniVal->getLimitedValue();
        if (IV == 0) {
          Elt = llvm::ConstantInt::get(cast<llvm::FixedVectorType>(DstTy)->getElementType(), 0);
        } else {
          Elt = llvm::ConstantInt::get(cast<llvm::FixedVectorType>(DstTy)->getElementType(), 1);
        }
      } else {
        Elt = CGF.Builder.CreateICmp(llvm::CmpInst::ICMP_NE, Elt, llvm::ConstantInt::get(CGF.Int8Ty, 0));
      }
    }
#endif
