  ...
  for (unsigned i = 0; i != NumInitElements; ++i) {
    ...
    if (!VVT) {
      ...
      if (CGF.getTarget().getTriple().getArch() == llvm::Triple::tpc &&
          ...
          cast<llvm::IntegerType>(VType->getVectorElementType())->getBitWidth() == 1 &&
          ResElts == 256) {
        ...
        if (const auto *IniVal = dyn_cast<llvm::ConstantInt>(Init)) {
          ...
          if (IV == 0) {
            Init = llvm::ConstantInt::get(VType->getVectorElementType(), 0);
          } else if (IV == 0xff) {
            Init = llvm::ConstantInt::get(VType->getVectorElementType(), 1);
          } else {
...
Value *ScalarExprEmitter::VisitCastExpr(CastExpr *CE) {
  ...
  case CK_VectorSplat: {
    ...
    if (CGF.getTarget().getTriple().getArch() == llvm::Triple::tpc &&
        DstTy->isVectorTy() &&
        DstTy->getVectorElementType()->isIntegerTy() &&
        cast<llvm::IntegerType>(DstTy->getVectorElementType())->getBitWidth() == 1 &&
        DstTy->getVectorNumElements() == 256) {
      // bool256 is represented as <32 x i32> in frontend so its initializer has type
      // i32. Convert it to i1.
      if (const auto *IniVal = dyn_cast<llvm::ConstantInt>(Elt)) {
        uint64_t IV = IniVal->getLimitedValue();
        if (IV == 0) {
          Elt = llvm::ConstantInt::get(DstTy->getVectorElementType(), 0);
        } else {
          Elt = llvm::ConstantInt::get(DstTy->getVectorElementType(), 1);
        }
      } else {
