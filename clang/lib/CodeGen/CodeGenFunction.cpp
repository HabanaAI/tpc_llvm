...
#undef SANITIZER
  ...
  if (const FunctionDecl *FD = dyn_cast_or_null<FunctionDecl>(D)) {
    if (FD->hasAttr<IndexSpaceAttr>()) {
      llvm::SmallVector<llvm::Metadata *, 16> IndexSpaceMDArgs;
      llvm::LLVMContext &Ctx = getLLVMContext();

      for (const IndexSpaceAttr *ISA : FD->specific_attrs<IndexSpaceAttr>()) {
        assert(ISA != nullptr && "Index space attribute should be there.");
        // Create metadata for tensor IDs.
        for (unsigned ID : ISA->tensor_ids())
          IndexSpaceMDArgs.push_back(
              llvm::ConstantAsMetadata::get(Builder.getInt32(ID)));

        // Create metadata for index space factors.
        for (auto IF : ISA->index_factors())
          IndexSpaceMDArgs.push_back(llvm::MDString::get(Ctx, IF));
      }
      Fn->setMetadata("index_space", llvm::MDNode::get(Ctx, IndexSpaceMDArgs));
    }

    if (FD->hasAttr<IndexSpacebAttr>()) {
      llvm::SmallVector<llvm::Metadata *, 16> IndexSpacebMDArgs;
      llvm::LLVMContext &Ctx = getLLVMContext();

      for (const IndexSpacebAttr *ISA : FD->specific_attrs<IndexSpacebAttr>()) {
        assert(ISA != nullptr && "Index space attribute should be there.");
        // Create metadata for tensor IDs.
        for (unsigned ID : ISA->tensor_ids())
          IndexSpacebMDArgs.push_back(
              llvm::ConstantAsMetadata::get(Builder.getInt32(ID)));

        // Create metadata for index spaceb factors.
        for (auto IF : ISA->index_factors())
          IndexSpacebMDArgs.push_back(llvm::MDString::get(Ctx, IF));
      }
      Fn->setMetadata("index_space_b",
                      llvm::MDNode::get(Ctx, IndexSpacebMDArgs));
    }

    if (FD->hasAttr<ReductionOrNormAxesAttr>()) {
      llvm::SmallVector<llvm::Metadata *, 16> ReductionOrNormAxesMDArgs;
      llvm::LLVMContext &Ctx = getLLVMContext();

      for (const ReductionOrNormAxesAttr *RNAA :
           FD->specific_attrs<ReductionOrNormAxesAttr>()) {
        assert(RNAA != nullptr &&
               "ReductionOrNormAxes attribute should be there.");
        for (unsigned ID : RNAA->tensor_ids())
          ReductionOrNormAxesMDArgs.push_back(
              llvm::ConstantAsMetadata::get(Builder.getInt32(ID)));
        for (auto Axis : RNAA->axes())
          ReductionOrNormAxesMDArgs.push_back(llvm::MDString::get(Ctx, Axis));
      }
      Fn->setMetadata("reduction_or_norm_axes",
                      llvm::MDNode::get(Ctx, ReductionOrNormAxesMDArgs));
    }
  }
#endif
