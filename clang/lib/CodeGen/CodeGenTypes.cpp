...
static llvm::Type *getTypeForFormat(llvm::LLVMContext &VMContext,
  ...
  if (&format == &llvm::APFloat::BFloat16())
    return llvm::Type::getBFloat16Ty(VMContext);
  if (&format == &llvm::APFloat::Float8_152())
    return llvm::Type::getF8_152Ty(VMContext);
  if (&format == &llvm::APFloat::Float8_143())
    return llvm::Type::getF8_143Ty(VMContext);
#endif
...
#ifdef LLVM_TPC_COMPILER
    if (getTarget().getTriple().getArch() == llvm::Triple::tpc) {
      // Create v256i1 for bool256, bool128 and bool64.
      if (VT->getElementType()->getAs<BuiltinType>()->getKind() == BuiltinType::UChar)
        switch (VT->getNumElements()) {
        case 32:
        case 16:
        case 8:
          ResultType = llvm::VectorType::get(llvm::Type::getInt1Ty(getLLVMContext()), 256);
          break;
        default:
          break;
        }
