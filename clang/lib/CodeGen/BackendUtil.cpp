...
static bool initTargetOptions(DiagnosticsEngine &Diags,
  ...
  Options.MCOptions.MCCompressInst = CodeGenOpts.CompressInstructions;
#endif
...
#ifdef LLVM_TPC_COMPILER
  if (CodeGenOpts.OptimizationLevel <= 1 || TargetTriple.getArch() == Triple::tpc) {
#else
...
#ifdef LLVM_TPC_COMPILER
  if (StringRef(TargetOpts.Triple).startswith("tpc"))
    PMBuilder.IsTPCTarget = true;
#endif
...
void EmitAssemblyHelper::CreateTargetMachine(bool MustCreateTM) {
  ...
  llvm::TargetOptions Options;
#ifdef LLVM_TPC_COMPILER
  Options.LocalScalarMemory = CodeGenOpts.ScalarLocalMemory;
  Options.LocalVectorMemory = CodeGenOpts.VectorLocalMemory;
  Options.SpillVlm = CodeGenOpts.SpillVlm;
  Options.AllLoopsTaken = CodeGenOpts.AllLoopsTaken;
  Options.RegMemCount = CodeGenOpts.RegMemCount;
  Options.LutWarn = CodeGenOpts.LutWarn;
  Options.CompressInstructions = CodeGenOpts.CompressInstructions;
  Options.LongIRF = CodeGenOpts.LongIRF;
  Options.TpcDnorm = CodeGenOpts.TpcDnorm;
  Options.MaxTensor = CodeGenOpts.MaxTensor;
  Options.CompileForLibrary = CodeGenOpts.CompileForLibrary;
#endif
...
#ifdef LLVM_TPC_COMPILER
void clang::EmbedText(llvm::Module *M, const CodeGenOptions &CGOpts,
                      StringRef Str, StringRef SectionName, StringRef ItemName) {
  ArrayRef<uint8_t> TheText(
      reinterpret_cast<uint8_t *>(const_cast<char *>(Str.data())), Str.size());
  llvm::Constant *TextConstant =
    llvm::ConstantDataArray::get(M->getContext(), TheText);
  auto GV = new llvm::GlobalVariable(*M, TextConstant->getType(), true,
                                     llvm::GlobalValue::PrivateLinkage,
                                     TextConstant);
  GV->setSection(SectionName);
  Type *TextType = Type::getInt8Ty(M->getContext())->getPointerTo(0);
  Constant* TextConstantPtr = ConstantExpr::getPointerBitCastOrAddrSpaceCast(GV, TextType);
  if (llvm::GlobalVariable *Old =
      M->getGlobalVariable(ItemName, true)) {
  ...
  ArrayType *ATy = ArrayType::get(TextType, 1);
  auto *NewUsed = new GlobalVariable(
    *M, ATy, false, llvm::GlobalValue::AppendingLinkage,
    llvm::ConstantArray::get(ATy, TextConstantPtr), "llvm.compiler.used");
