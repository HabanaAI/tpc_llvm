...
CodeGenModule::CodeGenModule(ASTContext &C, const HeaderSearchOptions &HSO,
  ...
  BFloat16Ty = llvm::Type::getBFloat16Ty(LLVMContext);
  Float8_143Ty = llvm::Type::getF8_143Ty(LLVMContext);
  Float8_152Ty = llvm::Type::getF8_152Ty(LLVMContext);
#endif
...
#ifdef LLVM_TPC_COMPILER
  if (CodeGenOpts.MarkAsUsingPrintf || Context.isPrintfIsUsed())
    getModule().addModuleFlag(llvm::Module::Override, "tpc-printf",
                              Context.getPrintfTensor());
#endif
...
#ifdef LLVM_TPC_COMPILER
  // Pass the attribute from declaration to the function.
  if (D && D->hasAttr<IndexSpaceAttr>())
    B.addAttribute(llvm::Attribute::IndexSpace);

  if (D && D->hasAttr<IndexSpacebAttr>())
    B.addAttribute(llvm::Attribute::IndexSpaceb);

  if (D && D->hasAttr<ReductionOrNormAxesAttr>())
    B.addAttribute(llvm::Attribute::ReductionOrNormAxes);
#endif
...
#ifdef LLVM_TPC_COMPILER
  // No not make zero initialization in the case of TPC core,
  // as per GAUDI-273/#2332.
  else if (!InitExpr && getTarget().getTriple().getArch() == llvm::Triple::tpc) {
