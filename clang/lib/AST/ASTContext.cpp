...
void ASTContext::InitBuiltinTypes(const TargetInfo &Target,
  ...
  InitBuiltinType(Float8_143Ty,            BuiltinType::Float8_143);
  InitBuiltinType(Float8_152Ty,            BuiltinType::Float8_152);
#endif
...
#ifdef LLVM_TPC_COMPILER
  case BuiltinType::Float8_143: return Target->getFloat8_143Format();
  case BuiltinType::Float8_152: return Target->getFloat8_152Format();
  case BuiltinType::BFloat16:   return Target->getBFloat16Format();
#endif
...
#include "clang/AST/TypeNodes.inc"
...
#ifdef LLVM_TPC_COMPILER
    if (Target->getTriple().getArch() == llvm::Triple::tpc) {
      if (VT->getNumElements() == 5) {
        Align = EltInfo.Align;
        break;
      }
    }
  ...
  case Type::Builtin:
    ...
    case BuiltinType::Float8_152:
      Width = Target->getFloat8Width();
      Align = Target->getFloat8Align();
      break;
...
#ifdef LLVM_TPC_COMPILER
  case BuiltinType::Float8_143: return Float8_143Rank;
  case BuiltinType::Float8_152: return Float8_152Rank;
...
QualType ASTContext::getFloatingTypeOfSizeWithinDomain(QualType Size,
  ...
  case Float8_143Rank: return Float8_143Ty;
  case Float8_152Rank: return Float8_152Ty;
#endif
...
static QualType DecodeTypeFromStr(const char *&Str, const ASTContext &Context,
  ...
  case 'B':
    assert(HowLong == 0 && !Signed && !Unsigned &&
           "Bad modifiers used with 'B'!");
    Type = Context.BFloat16Ty;
    break;
  case 'Q':
    assert(HowLong == 0 && !Signed && !Unsigned &&
           "Bad modifiers used with 'Q'!");
    Type = Context.Float8_152Ty;
    break;
  case 'q':
    assert(HowLong == 0 && !Signed && !Unsigned &&
           "Bad modifiers used with 'q'!");
    Type = Context.Float8_143Ty;
    break;
#endif
  case 'h':
    assert(HowLong == 0 && !Signed && !Unsigned &&
  ...
  case 'R': case 'r': {
    char *End;
    unsigned RecordNum = strtoul(Str, &End, 10);
    assert(End != Str && "Missing record number");
    Str = End;

    const TypeDecl *TD = Context.getTpcRecordDecl(RecordNum);
    // If the record type is not defined, probably it a type unavailable on this
    // platform, like bfloat128 on Goya.
    if (!TD) {
      Error = ASTContext::GE_Missing_stdio;
      return{};
    }
    Type = Context.getTypeDeclType(TD);
    break;
  }
#endif
  case 'E': {
...
struct TypeDescriptor {
  ...
  TpcType    Code;          // ... used to reference this type.
  unsigned   FeatureMask;   // Features required for this type.
};

const unsigned Feature_BFloat16 = 0x01;
const unsigned Feature_Half     = 0x02;
const unsigned Feature_Float8   = 0x04;

// Database of types that must be known to frontend.
//
// Note: order of this declarations is important. Index in this table is used as
// an identifier of a structure type parameter in intrinsic descriptions.
//
static const TypeDescriptor PredefinedTypes[] = {
  { "float64_pair_t",             TpcType::float128,            0 }, // r0
  { "float64_int64_pair_t",       TpcType::float64_int64,       0 }, // r1
  { "float64_uint64_pair_t",      TpcType::float64_uint64,      0 }, // r2

  { "int64_float64_pair_t",       TpcType::int64_float64,       0 }, // r3
  { "int64_pair_t",               TpcType::int128,              0 }, // r4
  { "int64_uint64_pair_t",        TpcType::int64_uint64,        0 }, // r5

  { "uint64_float64_pair_t",      TpcType::uint64_float64,      0 }, // r6
  { "uint64_int64_pair_t",        TpcType::uint64_int64,        0 }, // r7
  { "uint64_pair_t",              TpcType::uint128,             0 }, // r8

  { "bfloat128_pair_t",           TpcType::bfloat256,           Feature_BFloat16 }, // r9
  { "bfloat128_half128_pair_t",   TpcType::bfloat128_half128,   Feature_BFloat16 | Feature_Half }, // r10
  { "bfloat128_short128_pair_t",  TpcType::bfloat128_short128,  Feature_BFloat16 }, // r11
  { "bfloat128_ushort128_pair_t", TpcType::bfloat128_ushort128, Feature_BFloat16 }, // r12

  { "half128_bfloat128_pair_t",   TpcType::half128_bfloat128,   Feature_BFloat16 | Feature_Half }, // r13
  { "half128_pair_t",             TpcType::half256,             Feature_Half }, // r14
  { "half128_short128_pair_t",    TpcType::half128_short128,    Feature_Half }, // r15
  { "half128_ushort128_pair_t",   TpcType::half128_ushort128,   Feature_Half }, // r16

  { "short128_bfloat128_pair_t",  TpcType::short128_bfloat128,  Feature_BFloat16 }, // r17
  { "short128_half128_pair_t",    TpcType::short128_half128,    Feature_Half }, // r18
  { "short128_pair_t",            TpcType::short256,            0 }, // r19
  { "short128_ushort128_pair_t",  TpcType::short128_bfloat128,  0 }, // r20

  { "ushort128_bfloat128_pair_t", TpcType::ushort128_bfloat128, Feature_BFloat16 }, // r21
  { "ushort128_half128_pair_t",   TpcType::ushort128_half128,   Feature_Half }, // r22
  { "ushort128_short128_pair_t",  TpcType::ushort128_short128,  0 }, // r23
  { "ushort128_pair_t",           TpcType::ushort256,           0 }, // r24

  { "char256_pair_t",             TpcType::char512,             0 }, // r25
  { "char256_uchar256_pair_t",    TpcType::char256_uchar256,    0 }, // r26
  { "char256_minifloat256_pair_t", TpcType::char256_minifloat256, Feature_Float8 }, // r27
  { "char256_minihalf256_pair_t",  TpcType::char256_minihalf256,  Feature_Float8 }, // r28

  { "uchar256_char256_pair_t",    TpcType::uchar256_char256,    0 }, // r29
  { "uchar256_pair_t",            TpcType::char512,             0 }, // r30
  { "uchar256_minifloat256_pair_t", TpcType::uchar256_minifloat256, Feature_Float8 }, // r31
  { "uchar256_minihalf256_pair_t",  TpcType::uchar256_minihalf256,  Feature_Float8 }, // r32

  { "uint32_t_pair_t",            TpcType::uint32x2,            0 }, // r33
  { "uint16_t_pair_t",            TpcType::uint16x2,            0 }, // r34
  { "uint8_t_pair_t",             TpcType::uint8x2,             0 }, // r35

  { "int256",                     TpcType::int256,              0 }, // r36
  { "uint256",                    TpcType::uint256,             0 }, // r37
  { "float256",                   TpcType::float256,            0 }, // r38

  { "minifloat256_pair_t",             TpcType::minifloat512,             Feature_Float8 }, // r39
  { "minifloat256_minihalf256_pair_t", TpcType::minifloat256_minihalf256, Feature_Float8 }, // r40
  { "minifloat256_char256_pair_t",     TpcType::minifloat256_char256,     Feature_Float8 }, // r41
  { "minifloat256_uchar256_pair_t",    TpcType::minifloat256_uchar256,    Feature_Float8 }, // r42

  { "minihalf256_pair_t",              TpcType::minihalf512,              Feature_Float8 }, // r43
  { "minihalf256_minifloat256_pair_t", TpcType::minihalf256_minifloat256, Feature_Float8 }, // r44
  { "minihalf256_char256_pair_t",      TpcType::minihalf256_char256,      Feature_Float8 }, // r45
  { "minihalf256_uchar256_pair_t",     TpcType::minihalf256_uchar256,     Feature_Float8 }, // r46

  {"int32_t_pair_t",                   TpcType::int32x2,                  0}, //r47

  // The following entries represent vector types, their order is not important.

  { "bool256",                    TpcType::bool256,             0 },
  { "bool128",                    TpcType::bool128,             0 },
  { "bool64",                     TpcType::bool64,              0 }
};

// Maps structure number (used in intrinsic descriptions) to type declaration.
static std::vector<const TypeDecl *>  TpcTypeDecls;

// Maps numerical codes to corresponding type declarations.
static std::map<TpcType, const TypeDecl *> TpcTypes;


void ASTContext::loadTpcTypes() const {
  if (!TpcTypeDecls.empty())
    return;

  // Define feature set available.
  unsigned FeatureMask = 0;
  if (getLangOpts().BFloat16)
    FeatureMask |= Feature_BFloat16;
  if (getLangOpts().Half)
    FeatureMask |= Feature_Half;
  if (getLangOpts().Float8)
    FeatureMask |= Feature_Float8;

  // Load predefined types and put corresponding entries into helper tables.
  for (unsigned I = 0, E = llvm::array_lengthof(PredefinedTypes); I < E; ++I) {
    // Filter types by features.
    const unsigned ThisFeatureMask = PredefinedTypes[I].FeatureMask;
    if ((FeatureMask & ThisFeatureMask) != ThisFeatureMask) {
      assert(I == TpcTypeDecls.size());
      TpcTypeDecls.push_back(nullptr);
      continue;
    }

    // Load type.
    IdentifierInfo *II = &Idents.get(PredefinedTypes[I].Name);
    DeclarationName Name(II);
    DeclContext::lookup_result DR = TUDecl->lookup(Name);
    assert(DR.size() == 1);
    assert(I == TpcTypeDecls.size());
    auto TD = cast<TypeDecl>(DR.front());

    // Put the type into tables.
    TpcTypeDecls.push_back(TD);
    TpcTypes[PredefinedTypes[I].Code] = TD;
  }
}


const TypeDecl *ASTContext::getTpcTypeDecl(TpcType T) const {
  auto Ptr = TpcTypes.find(T);
  if (Ptr == TpcTypes.end())
    return nullptr;
  return Ptr->second;
}

QualType ASTContext::getTpcType(TpcType T) const {
  if (const TypeDecl *TD = getTpcTypeDecl(T)) {
    return getTypeDeclType(TD).getCanonicalType();
  }
  return QualType();
}

const TypeDecl *ASTContext::getTpcRecordDecl(unsigned R) const {
  const TypeDecl *TD = TpcTypeDecls[R];
  return TD;
}
#endif
