...
#include "TreeTransform.h" // LLVM_TPC_COMPILER
...
#ifdef LLVM_TPC_COMPILER
      Expr *DefArg = buildDefaultArgument(FT, ID, i);
#endif
      ParmVarDecl *parm = ParmVarDecl::Create(
          Context, New, SourceLocation(), SourceLocation(), nullptr,
          FT->getParamType(i), /*TInfo=*/nullptr, SC_None, /*LLVM_TPC_COMPILER*/DefArg);
...
#ifdef LLVM_TPC_COMPILER
    if (LangOpts.RC99) {
      Diag(Loc, diag::err_unavailable_intrinsic)
...
#ifdef LLVM_TPC_COMPILER
      if (!(Context.getTargetInfo().getTriple().getArch() == llvm::Triple::ArchType::tpc
            && R->isVectorType()))
#endif
...
void Sema::CheckMain(FunctionDecl* FD, const DeclSpec& DS) {
  ...
  if (getLangOpts().GNUMode && !getLangOpts().CPlusPlus) {
    ...
    if (Context.hasSameType(FT->getReturnType(), Context.IntTy))
      FD->setHasImplicitReturnZero(true);
    else if (/*LLVM_TPC_COMPILER*/ !getLangOpts().RC99) {
