...
Sema::BuildCXXNamedCast(SourceLocation OpLoc, tok::TokenKind Kind,
  ...
  case tok::kw_dynamic_cast: {
    ...
    if (getLangOpts().RCXX) {
      return ExprError(Diag(OpLoc, diag::err_rcxx_not_supported)
                       << "dynamic_cast");
    }
#endif
...
#ifdef LLVM_TPC_COMPILER
  if (Self.getLangOpts().RC99 && SrcType->isPointerType()) {
    QualType Ty = SrcType->getPointeeType();
    QualType TD = DestType;
    if (TD->isPointerType())
      TD = TD->getPointeeType();

    if (Ty->isVectorType() != TD->isVectorType()) {
      if (Ty->isVectorType())
      Self.Diag(OpRange.getBegin(), diag::err_invalid_conversion_between_vector_and_scalar)
        << Ty << TD << SrcExpr->getSourceRange();
      else 
      Self.Diag(OpRange.getBegin(), diag::err_invalid_conversion_between_scalar_and_vector)
        << Ty << TD << SrcExpr->getSourceRange();
    }
  }
#endif
