...
ExprResult Sema::DefaultArgumentPromotion(Expr *E) {
  ...
  // half/fp16); it does not apply to _Float16 (and _BFloat16).
...
#ifdef LLVM_TPC_COMPILER
  {
    bool LHSisBF16 = false, RHSisBF16 = false;
    if (const auto *BT = dyn_cast<BuiltinType>(LHSType->getCanonicalTypeUnqualified()))
      if (BT->getKind() == BuiltinType::BFloat16)
        LHSisBF16 = true;
    if (const auto *BT = dyn_cast<BuiltinType>(RHSType->getCanonicalTypeUnqualified()))
      if (BT->getKind() == BuiltinType::BFloat16)
        RHSisBF16 = true;
    if (!getLangOpts().BFloat16 && (LHSisBF16 || RHSisBF16))
      return QualType();
  }
#endif
...
ExprResult Sema::ActOnNumericConstant(const Token &Tok, Scope *UDLScope) {
  ...
  if (Literal.isFixedPointLiteral()) {
    ...
    QualType Ty;
    if (Literal.isHalf){
      if (getOpenCLOptions().isEnabled("cl_khr_fp16") /*LLVM_TPC_COMPILER*/ || getLangOpts().RC99)
        Ty = Context.HalfTy;
      else {
...
#ifdef LLVM_TPC_COMPILER
    if (Ty == Context.Float128Ty && LangOpts.RC99) {
      Diag(Tok.getLocation(), diag::err_float128_const_not_alloved);
      return ExprError();
    }
#endif
...
#ifdef LLVM_TPC_COMPILER
      if (getLangOpts().RC99 && !isPrintfEnabled(FDecl)) {
        auto Zero = IntegerLiteral::Create(Context,
            llvm::APInt(Context.getIntWidth(Context.IntTy), 0), Context.IntTy,
            LParenLoc);
        return CStyleCastExpr::Create(Context, Context.VoidTy, VK_RValue,
            CK_ToVoid, Zero, nullptr, FPOptionsOverride(),
            Context.getTrivialTypeSourceInfo(Context.IntTy, LParenLoc),
            LParenLoc, LParenLoc);
      }
#endif
...
#ifdef LLVM_TPC_COMPILER
    bool LHSisBF16 = false, RHSisBF16 = false;
    if (const auto *BT = dyn_cast<BuiltinType>(LHSTy->getCanonicalTypeUnqualified()))
      if (BT->getKind() == BuiltinType::BFloat16)
        LHSisBF16 = true;
    if (const auto *BT = dyn_cast<BuiltinType>(RHSTy->getCanonicalTypeUnqualified()))
      if (BT->getKind() == BuiltinType::BFloat16)
        RHSisBF16 = true;
    if (getLangOpts().BFloat16 || (!LHSisBF16 && !RHSisBF16)) {
#endif
...
#ifdef LLVM_TPC_COMPILER
  const auto isNonArithmeticBFloat = [=](QualType T)->bool {
    if (const auto *BT = dyn_cast<BuiltinType>(T->getCanonicalTypeUnqualified()))
      if (BT->getKind() == BuiltinType::BFloat16)
        return !getLangOpts().BFloat16;
    return false;
  };
#endif

  if (compType.isNull() || !compType->isArithmeticType() ||
      /*LLVM_TPC_COMPILER*/ isNonArithmeticBFloat(compType))
    return InvalidOperands(Loc, LHS, RHS);
  if (IsDiv) {
...
#ifdef LLVM_TPC_COMPILER
    bool IsBF16 = false;
    if (const auto *BT = dyn_cast<BuiltinType>(compType->getCanonicalTypeUnqualified()))
      if (BT->getKind() == BuiltinType::BFloat16)
        IsBF16 = true;
    if (getLangOpts().BFloat16 || !IsBF16) {
#endif
    if (CompLHSTy) *CompLHSTy = compType;
    return compType;
#ifdef LLVM_TPC_COMPILER
...
#ifdef LLVM_TPC_COMPILER
  } else if(S.getLangOpts().RC99 &&
            ResType->isVectorType() &&
...
QualType Sema::CheckAddressOfOperand(ExprResult &OrigOp, SourceLocation OpLoc) {
  ...
  if (lval == Expr::LV_ClassTemporary || lval == Expr::LV_ArrayTemporary) {
    ...
    if (!op->getType()->isFunctionType()) {
      ...
      Diag(op->getExprLoc(), diag::err_tpc_pointer_to_function);
#endif
