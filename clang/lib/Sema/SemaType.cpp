...
static QualType ConvertDeclSpecToType(TypeProcessingState &state) {
  ...
  case DeclSpec::TST_int: {
    if (DS.getTypeSpecSign() != DeclSpec::TSS_unsigned) {
      ...
      case DeclSpec::TSW_longlong:
        ...
        if (S.getLangOpts().RC99) {
          S.Diag(DS.getTypeSpecWidthLoc(), diag::err_type_unsupported)
                 << "long long";
          declarator.setInvalidType(true);
        }
#endif
  ...
  case DeclSpec::TST_double:
...
#ifdef LLVM_TPC_COMPILER
    if (S.getLangOpts().RC99) {
      if (Result == Context.LongDoubleTy)
        S.Diag(DS.getTypeSpecWidthLoc(), diag::err_type_unsupported)
               << "long double";
      else
        S.Diag(DS.getTypeSpecTypeLoc(), diag::err_type_unsupported)
    ...
    break;
#ifdef LLVM_TPC_COMPILER
  case DeclSpec::TST_bfloat16: Result = Context.BFloat16Ty; break;
  case DeclSpec::TST_float8_152: Result = Context.Float8_152Ty; break;
  case DeclSpec::TST_float8_143: Result = Context.Float8_143Ty; break;
#endif
  ...
  for (unsigned i = 0, e = D.getNumTypeObjects(); i != e; ++i) {
    ...
    case DeclaratorChunk::Pointer:
      ...
      if (T->getAs<FunctionType>())
        if (LangOpts.RC99) {
          S.Diag(D.getIdentifierLoc(), diag::err_tpc_pointer_to_function);
          D.setInvalidType(true);
        }
#endif
...
#ifdef LLVM_TPC_COMPILER
      if (LangOpts.RC99 && FTI.isVariadic ) {
        S.Diag(D.getIdentifierLoc(), diag::err_tpc_variadic_function);
        D.setInvalidType(true);
      }
#endif
  ...
  if (Attr.getKind() == ParsedAttr::AT_AddressSpace) {
    ...
    if (S.LangOpts.RC99) {
      ...
      case LangAS::opencl_global:
        ASIdx = LangAS::rc99_global;
        break;
      case LangAS::opencl_local:
        if (isInTPCVectorAddressSpace(Type))
          ASIdx = LangAS::rc99_local_vector;
        else
          ASIdx = LangAS::rc99_local_scalar;
        break;
      default:
        break;
      }
...
#ifdef LLVM_TPC_COMPILER
  // If address space is not set, RC99 considers type as referring to local
  // scalar address space.
  if (state.getSema().getLangOpts().RC99 &&
    ...
    if (DS.getStorageClassSpec() != DeclSpec::SCS_typedef &&
        !D.isFunctionDeclarator() && !(type->isArrayType())) {
      if (// If variable is global
          Ctx == DeclaratorContext::FileContext ||
          // or this is a local static variable declaration
          (Ctx == DeclaratorContext::BlockContext &&
           DS.getStorageClassSpec() == DeclSpec::SCS_static) ||
          // or this is a pointer declaration
           (state.getCurrentChunkIndex() > 0 &&
            D.getTypeObject(state.getCurrentChunkIndex() - 1).Kind ==
            DeclaratorChunk::Pointer)) {
        if (isInTPCVectorAddressSpace(type))
          AddrSpace = LangAS::rc99_local_vector;
        else
          AddrSpace = LangAS::rc99_local_scalar;
      }
    }
    if (AddrSpace != LangAS::Default && type.getAddressSpace() == LangAS::Default) {
