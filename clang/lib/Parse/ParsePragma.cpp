...
struct PragmaTpcPrintfHandler : public PragmaHandler {
  explicit PragmaTpcPrintfHandler() : PragmaHandler("tpc_printf") {}
  void HandlePragma(Preprocessor &PP, PragmaIntroducer Introducer,
                    Token &FirstToken) override;
};

struct PragmaLoopPipelineHintHandler : public PragmaHandler {
  PragmaLoopPipelineHintHandler(const char *name) : PragmaHandler(name) {}
  void HandlePragma(Preprocessor &PP, PragmaIntroducer Introducer,
                    Token &FirstToken) override;
};

struct PragmaIndexSpaceHandler : public PragmaHandler {
  explicit PragmaIndexSpaceHandler(const char *Name) : PragmaHandler(Name) {}
  void HandlePragma(Preprocessor &PP, PragmaIntroducer Introducer,
                    Token &FirstToken) override;
};

struct PragmaIndexSpacebHandler : public PragmaHandler {
  explicit PragmaIndexSpacebHandler(const char *Name) : PragmaHandler(Name) {}
  void HandlePragma(Preprocessor &PP, PragmaIntroducer Introducer,
                    Token &FirstToken) override;
};

struct PragmaReductionOrNormAxesHandler : public PragmaHandler {
  explicit PragmaReductionOrNormAxesHandler(const char *Name)
      : PragmaHandler(Name) {}
  void HandlePragma(Preprocessor &PP, PragmaIntroducer Introducer,
                    Token &FirstToken) override;
};
...
#ifdef LLVM_TPC_COMPILER
  Tpc_printfHandler.reset(new PragmaTpcPrintfHandler());
  PP.AddPragmaHandler(Tpc_printfHandler.get());

  LoopUnrollHintHandler.reset(new PragmaUnrollHintHandler("loop_unroll"));
  PP.AddPragmaHandler(LoopUnrollHintHandler.get());

  LoopUnrollDivideHintHandler.reset(new PragmaUnrollHintHandler("loop_unroll_divide"));
  PP.AddPragmaHandler(LoopUnrollDivideHintHandler.get());

  LoopTakenHintHandler.reset(new PragmaUnrollHintHandler("loop_taken"));
  PP.AddPragmaHandler(LoopTakenHintHandler.get());

  LoopPipelineHintHandler.reset(
      new PragmaLoopPipelineHintHandler("loop_pipeline"));
  PP.AddPragmaHandler(LoopPipelineHintHandler.get());

  IndexSpaceHandler.reset(new PragmaIndexSpaceHandler("index_space"));
  PP.AddPragmaHandler(IndexSpaceHandler.get());

  IndexSpacebHandler.reset(new PragmaIndexSpacebHandler("index_space_b"));
  PP.AddPragmaHandler(IndexSpacebHandler.get());

  ReductionAxesHandler.reset(
      new PragmaReductionOrNormAxesHandler("reduction_axes"));
  PP.AddPragmaHandler(ReductionAxesHandler.get());

  NormAxesHandler.reset(new PragmaReductionOrNormAxesHandler("norm_axes"));
  PP.AddPragmaHandler(NormAxesHandler.get());
#endif
...
void Parser::resetPragmaHandlers() {
  ...
  PackHandler.reset();
#ifdef LLVM_TPC_COMPILER
  PP.RemovePragmaHandler(Tpc_printfHandler.get());
  Tpc_printfHandler.reset();
  PP.RemovePragmaHandler(LoopUnrollHintHandler.get());
  LoopUnrollHintHandler.reset();
  PP.RemovePragmaHandler(LoopUnrollDivideHintHandler.get());
  LoopUnrollDivideHintHandler.reset();
  PP.RemovePragmaHandler(LoopTakenHintHandler.get());
  LoopTakenHintHandler.reset();
  PP.RemovePragmaHandler(LoopPipelineHintHandler.get());
  LoopPipelineHintHandler.reset();
  PP.RemovePragmaHandler(IndexSpaceHandler.get());
  IndexSpaceHandler.reset();
  PP.RemovePragmaHandler(IndexSpacebHandler.get());
  IndexSpacebHandler.reset();
  PP.RemovePragmaHandler(ReductionAxesHandler.get());
  ReductionAxesHandler.reset();
  PP.RemovePragmaHandler(NormAxesHandler.get());
  NormAxesHandler.reset();
#endif
...
#ifdef LLVM_TPC_COMPILER
namespace {
  struct PragmaTpcPrintfInfo {
    Sema::PragmaTpcPrintfAction Action;
  };

  struct PragmaIndexSpaceInfo {
    llvm::SmallVector<unsigned, 16> TensorIDs;
    llvm::SmallVector<llvm::SmallString<128>, 5> IndexFactors;
  };

  struct PragmaIndexSpacebInfo {
    llvm::SmallVector<unsigned, 16> TensorIDs;
    llvm::SmallVector<llvm::SmallString<128>, 10> IndexFactors;
  };

  struct PragmaReductionOrNormAxesInfo {
    llvm::SmallVector<unsigned, 16> TensorIDs;
...
void Parser::HandlePragmaTpc_printf() {
  assert(Tok.is(tok::annot_pragma_tpc_printf));
  PragmaTpcPrintfInfo *Info = 
    static_cast<PragmaTpcPrintfInfo *>(Tok.getAnnotationValue());
  ConsumeAnnotationToken();
  Actions.ActOnPragmaTpcPrintf(Info->Action);
}

void Parser::HandlePragmaIndexSpace() {
  assert(Tok.is(tok::annot_pragma_index_space));
  PragmaIndexSpaceInfo *Info =
      static_cast<PragmaIndexSpaceInfo *>(Tok.getAnnotationValue());
  SourceLocation IndexSpaceLoc = ConsumeAnnotationToken();
  Actions.ActOnPragmaIndexSpace(getCurScope(), IndexSpaceLoc, Info->TensorIDs,
                                Info->IndexFactors);
}

void Parser::HandlePragmaIndexSpaceb() {
  assert(Tok.is(tok::annot_pragma_index_space_b));
  PragmaIndexSpacebInfo *Info =
      static_cast<PragmaIndexSpacebInfo *>(Tok.getAnnotationValue());
  SourceLocation IndexSpacebLoc = ConsumeAnnotationToken();
  Actions.ActOnPragmaIndexSpaceb(getCurScope(), IndexSpacebLoc, Info->TensorIDs,
                                 Info->IndexFactors);
}

void Parser::HandlePragmaReductionOrNormAxes() {
  assert(Tok.is(tok::annot_pragma_reduction_or_norm_axes));
  PragmaReductionOrNormAxesInfo *Info =
      static_cast<PragmaReductionOrNormAxesInfo *>(Tok.getAnnotationValue());
  SourceLocation ReductionOrNormAxesLoc = ConsumeAnnotationToken();
  Actions.ActOnPragmaReductionOrNormAxes(getCurScope(), ReductionOrNormAxesLoc,
                                         Info->TensorIDs, Info->Axes);
}
#endif
...
bool Parser::HandlePragmaLoopHint(LoopHint &Hint) {
...
#ifdef LLVM_TPC_COMPILER
  bool PragmaLoopUnroll = PragmaNameInfo->getName() == "loop_unroll";
  bool PragmaLoopUnrollDivide = PragmaNameInfo->getName() == "loop_unroll_divide";
  if (Toks.empty() && (PragmaLoopUnroll || PragmaLoopUnrollDivide)) {
    ConsumeAnnotationToken();
    Diag(Info->PragmaName.getLocation(), diag::err_pragma_loop_missing_argument)
      << /*StateArgument=*/0 << /*FullKeyword=*/0
      << /*AssumeSafetyKeyword=*/0;
    return false;
  }
#endif
...
#ifdef LLVM_TPC_COMPILER
                        .Cases("loop_unroll", "loop_unroll_divide",
                               "loop_taken", "loop_pipeline", true)
#endif
                        .Default(false);
  ...
  if (OptionInfo) { // Pragma Unroll does not specify an option.
    ...
    StateOption = llvm::StringSwitch<bool>(OptionInfo->getName())
                      ...
                      .Case("taken", true)
                      .Case("pipelined", true)
                      .Case("unaligned_trip_count", true)
#endif
...
#ifdef LLVM_TPC_COMPILER
    if (Tok.isNot(tok::eof) && (PragmaLoopUnroll || PragmaLoopUnrollDivide)) {
      while (Tok.is(tok::identifier)) {
        StringRef AttrName = Tok.getIdentifierInfo()->getName();
        if (AttrName.equals("taken")) {
          if (Hint.TakenLoc) {
            Diag(Tok.getLocation(), diag::warn_pragma_duplicate_option);
          } else {
            Hint.TakenLoc = IdentifierLoc::create(
                Actions.Context, Tok.getLocation(), Tok.getIdentifierInfo());
          }
          ConsumeToken();
        }
        else if (AttrName.equals("pipelined")) {
          if (Hint.PipelinedLoc) {
            Diag(Tok.getLocation(), diag::warn_pragma_duplicate_option);
          } else {
            Hint.PipelinedLoc = IdentifierLoc::create(
              Actions.Context, Tok.getLocation(), Tok.getIdentifierInfo());
          }
          ConsumeToken();
        }
        else if (AttrName.equals("unaligned_trip_count")) {
          if (Hint.UnalignedTripCountLoc) {
            Diag(Tok.getLocation(), diag::warn_pragma_duplicate_option);
          } else {
            Hint.UnalignedTripCountLoc = IdentifierLoc::create(
                Actions.Context, Tok.getLocation(), Tok.getIdentifierInfo());
          }
          ConsumeToken();
        }
...
#ifdef LLVM_TPC_COMPILER
// #pragma tpc_printf(enable/disable)
void PragmaTpcPrintfHandler::HandlePragma(Preprocessor &PP,
                                          PragmaIntroducer Introducer,
                                          Token &Tpc_printfTok) {
  SourceLocation Tpc_printfLoc = Tpc_printfTok.getLocation();

  Token Tok;
  PP.Lex(Tok);
  if (Tok.isNot(tok::l_paren)) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_expected_lparen) << "tpc_printf";
    return;
  }
  ...
  if (Tok.is(tok::identifier)) {
    const IdentifierInfo *II = Tok.getIdentifierInfo();
    if (II->isStr("enable")) {
      Action = Sema::PragmaTpcPrintfAction::Enable;
      PP.Lex(Tok);
    } else if (II->isStr("disable")) {
        Action = Sema::PragmaTpcPrintfAction::Disable;
        PP.Lex(Tok);
    } else {
      PP.Diag(Tok.getLocation(), diag::warn_pragma_invalid_action) << "tpc_printf";
      return;
    }

    if (Tok.isNot(tok::r_paren)) {
      PP.Diag(Tok.getLocation(), diag::warn_pragma_expected_rparen) << "tpc_printf";
      return;
    }
    ...
    PP.Lex(Tok);
    if (Tok.isNot(tok::eod)) {
      PP.Diag(Tok.getLocation(), diag::warn_pragma_extra_tokens_at_eol) << "tpc_printf";
      return;
    }

    PragmaTpcPrintfInfo *Info =
      PP.getPreprocessorAllocator().Allocate<PragmaTpcPrintfInfo>(1);
    Info->Action = Action;

    MutableArrayRef<Token> Toks(PP.getPreprocessorAllocator().Allocate<Token>(1),
      1);
    Toks[0].startToken();
    Toks[0].setKind(tok::annot_pragma_tpc_printf);
    Toks[0].setLocation(Tpc_printfLoc);
    Toks[0].setAnnotationEndLoc(RParenLoc);
    Toks[0].setAnnotationValue(static_cast<void*>(Info));
    PP.EnterTokenStream(Toks, /*DisableMacroExpansion=*/true, false);
  } else
    PP.Diag(Tok.getLocation(), diag::warn_pragma_tpc_printf_malformed);
}

/// Handle the loop pipeline optimization pragma.
///  #pragma loop_pipeline
///
/// Loop pipelining hint can be specified with '#pragma loop_pipeline'.
void PragmaLoopPipelineHintHandler::HandlePragma(Preprocessor &PP,
                                                 PragmaIntroducer Introducer,
                                                 Token &Tok) {
  // Incoming token is "loop_pipeline" for "#pragma loop_pipeline"
  Token PragmaName = Tok;
  PP.Lex(Tok);
...
void PragmaIndexSpaceHandler::HandlePragma(Preprocessor &PP,
                                           PragmaIntroducer Introducer,
                                           Token &Tok) {
  SourceLocation IndexSpaceLoc = Tok.getLocation();
  llvm::SmallVector<unsigned, 16> TensorIDs;
  llvm::SmallVector<llvm::SmallString<128>, 5> IndexFactors;

  Token PragmaName = Tok;
  auto PragmaStr = PragmaName.getIdentifierInfo()->getName();

  PP.Lex(Tok);
  if (Tok.isNot(tok::l_paren)) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_expected_lparen) << PragmaStr;
    return;
  }

  PP.Lex(Tok);
  // Parse the tensor IDs. It will be of the form
  // (1, 2, 3, 4..)
  while (Tok.isOneOf(tok::numeric_constant, tok::comma)) {
    if (Tok.is(tok::comma)) {
      PP.Lex(Tok);
      continue;
    }
    // Use an out of range value to identify parsing error.
    uint64_t TensorID = 100;
    PP.parseSimpleIntegerLiteral(Tok, TensorID);
    TensorIDs.push_back(TensorID);
  }

  if (Tok.isNot(tok::r_paren)) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_expected_rparen) << PragmaStr;
    return;
  }
  ...
  // Parse the index factors. It can be a formula or integer,
  // hence represented as string. It can be of the form.
  // ("x * y", "1", "1")
  while (Tok.isOneOf(tok::string_literal, tok::comma)) {
    if (Tok.is(tok::comma)) {
      PP.Lex(Tok);
      continue;
    }

    std::string IndexStr = PP.getSpelling(Tok);
    std::string Str = "";
    for (auto Ch : IndexStr)
      if (Ch != '"')
        Str += Ch;
    llvm::SmallString<128> IndexFactor(Str.begin(), Str.end());
    IndexFactors.push_back(IndexFactor);
    PP.Lex(Tok);
  }
  ...
  PragmaIndexSpaceInfo *Info = new PragmaIndexSpaceInfo;
  Info->TensorIDs = TensorIDs;
  Info->IndexFactors = IndexFactors;

  MutableArrayRef<Token> Toks(PP.getPreprocessorAllocator().Allocate<Token>(1),
                              1);
  Toks[0].startToken();
  Toks[0].setKind(tok::annot_pragma_index_space);
  Toks[0].setLocation(IndexSpaceLoc);
  Toks[0].setAnnotationEndLoc(IndexSpaceLoc);
  Toks[0].setAnnotationValue(static_cast<void *>(Info));
  PP.EnterTokenStream(Toks, /*DisableMacroExpansion=*/true, false);
}

void PragmaIndexSpacebHandler::HandlePragma(Preprocessor &PP,
                                            PragmaIntroducer Introducer,
                                            Token &Tok) {
  SourceLocation IndexSpacebLoc = Tok.getLocation();
  llvm::SmallVector<unsigned, 16> TensorIDs;
  llvm::SmallVector<llvm::SmallString<128>, 10> IndexFactors;
  ...
  PragmaIndexSpacebInfo *Info = new PragmaIndexSpacebInfo;
  Info->TensorIDs = TensorIDs;
  Info->IndexFactors = IndexFactors;
  ...
  MutableArrayRef<Token> Toks(PP.getPreprocessorAllocator().Allocate<Token>(1),
                              1);
  Toks[0].startToken();
  Toks[0].setKind(tok::annot_pragma_index_space_b);
  Toks[0].setLocation(IndexSpacebLoc);
  Toks[0].setAnnotationEndLoc(IndexSpacebLoc);
  Toks[0].setAnnotationValue(static_cast<void *>(Info));
  PP.EnterTokenStream(Toks, /*DisableMacroExpansion=*/true, false);
}

void PragmaReductionOrNormAxesHandler::HandlePragma(Preprocessor &PP,
                                                    PragmaIntroducer Introducer,
                                                    Token &Tok) {
  SourceLocation ReductionOrNormAxesLoc = Tok.getLocation();
  llvm::SmallVector<unsigned, 16> TensorIDs;
  llvm::SmallVector<llvm::SmallString<128>, 5> Axes;
  ...
  PP.Lex(Tok);
  // Parse the axis representation. It can be of the form.
  // ("%0", "1")
  while (Tok.isOneOf(tok::string_literal, tok::comma)) {
    if (Tok.is(tok::comma)) {
      PP.Lex(Tok);
    ...
    std::string AxisStr = PP.getSpelling(Tok);
    std::string Str = "";
    for (auto Ch : AxisStr)
      if (Ch != '"')
        Str += Ch;
    llvm::SmallString<128> Axis(Str.begin(), Str.end());
    Axes.push_back(Axis);
    PP.Lex(Tok);
  }
  ...
  PragmaReductionOrNormAxesInfo *Info =
      PP.getPreprocessorAllocator().Allocate<PragmaReductionOrNormAxesInfo>(1);
  Info->TensorIDs = TensorIDs;
  Info->Axes = Axes;
  ...
  MutableArrayRef<Token> Toks(PP.getPreprocessorAllocator().Allocate<Token>(1),
                              1);
  Toks[0].startToken();
  Toks[0].setKind(tok::annot_pragma_reduction_or_norm_axes);
  Toks[0].setLocation(ReductionOrNormAxesLoc);
  Toks[0].setAnnotationEndLoc(ReductionOrNormAxesLoc);
  Toks[0].setAnnotationValue(static_cast<void *>(Info));
  PP.EnterTokenStream(Toks, /*DisableMacroExpansion=*/true, false);
}
#endif
...
#ifdef LLVM_TPC_COMPILER
  if (Tok.isNot(tok::eod)) {
    if (PragmaName.getIdentifierInfo()->getName().equals("loop_unroll_divide") ||
        PragmaName.getIdentifierInfo()->getName().equals("loop_unroll")) {
      while (Tok.is(tok::identifier) &&
        (Tok.getIdentifierInfo()->getName().equals("taken") ||
         Tok.getIdentifierInfo()->getName().equals("pipelined") ||
         Tok.getIdentifierInfo()->getName().equals("unaligned_trip_count"))) {
        ValueList.push_back(Tok);
        PP.Lex(Tok);
      }
    }
...
#ifdef LLVM_TPC_COMPILER
                           .Case("machine_unroll_count", true)
                           .Case("divide", true)
                           .Case("taken", true)
                           .Case("pipelined", true)
                           .Case("unaligned_trip_count", true)
                           .Case("loop_pipeline", true)
#endif
                           .Case("pipeline", true)
...
///  #pragma unroll '(' unroll-hint-value ')'
///  #pragma loop_unroll '(' unroll-hint-value ')'
///  #pragma loop_unroll_divide '(' unroll-hint-value ')'
///  #pragma loop_taken
///  #pragma nounroll
