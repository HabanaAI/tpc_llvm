...
void Parser::ParseDeclarationSpecifiers(DeclSpec &DS,
  ...
  while (1) {
    ...
    case tok::kw_virtual:
      ...
      else if (getLangOpts().RCXX) {
        DiagID = diag::err_rcxx_virtual_function;
        PrevSpec = Tok.getIdentifierInfo()->getNameStart();
        isInvalid = true;
      }
#endif
    ...
    case tok::kw__Sat:
      ...
      break;
#ifdef LLVM_TPC_COMPILER
    case tok::kw__BFloat16:
      isInvalid = DS.SetTypeSpecType(DeclSpec::TST_bfloat16, Loc, PrevSpec,
                                     DiagID, Policy);
      break;
    case tok::kw__Float8_152:
      isInvalid = DS.SetTypeSpecType(DeclSpec::TST_float8_152, Loc, PrevSpec,
                                     DiagID, Policy);
      break;
    case tok::kw__Float8_143:
      isInvalid = DS.SetTypeSpecType(DeclSpec::TST_float8_143, Loc, PrevSpec,
                                     DiagID, Policy);
      break;
#endif
...
#ifdef LLVM_TPC_COMPILER
    if (Tok.is(tok::annot_pragma_tpc_printf)) {
      HandlePragmaTpc_printf();
      continue;
    }

    if (Tok.is(tok::annot_pragma_index_space)) {
      HandlePragmaIndexSpace();
      continue;
    }

    if (Tok.is(tok::annot_pragma_index_space_b)) {
      HandlePragmaIndexSpaceb();
      continue;
    }

    if (Tok.is(tok::annot_pragma_reduction_or_norm_axes)) {
      HandlePragmaReductionOrNormAxes();
      continue;
    }
#endif
...
void Parser::ParseFunctionDeclarator(Declarator &D,
  ...
  if (getLangOpts().RC99 &&
      Identifier && Identifier->getName() == getLangOpts().MainFunction) {
    for (unsigned i = 0; i < ParamInfo.size(); ) {
      VarDecl *Param = cast<VarDecl>(ParamInfo[i].Param);
      QualType AT = Param->getType();
      bool PassToNext = true;
      if (auto UserTy = AT->getAs<TypedefType>()) {
        if (!Param->isInvalidDecl() && // only correct declarations
			(UserTy->getDecl()->getNameAsString() == "tensor")) {
          Tensors.push_back(Param);
          ParamInfo.erase(ParamInfo.begin() + i);
          PassToNext = false;
        }
      }
      if (PassToNext)
    ...
    Actions.getASTContext().setPrintfTensor(Tensors.size());
    if (ParamInfo.size() > MaxMainArgs) {
      for (unsigned i = MaxMainArgs; i < ParamInfo.size(); ) {
        ExtraArgs.push_back(cast<ParmVarDecl>(ParamInfo[i].Param));
        ParamInfo.erase(ParamInfo.begin() + i);
      }
    }
