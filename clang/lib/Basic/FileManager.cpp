...
FileEntryRef FileManager::addVirtualFile(StringRef Filename, llvm::MemoryBuffer &Buff) {
  FileEntryRef FRef = getVirtualFileRef(Filename, Buff.getBufferSize(), 0);
  FileEntry &FE = const_cast<FileEntry &>(FRef.getFileEntry());
  FE.Content.reset(&Buff);
  return FRef;
}
