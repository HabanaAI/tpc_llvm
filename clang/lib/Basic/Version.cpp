...
std::string getTPCToolChain() {
#ifdef TPC_COMPILER_RELEASE
      return std::to_string(TPC_COMPILER_RELEASE());
#else
      return "";
#endif
    }
std::string getTPCDropVersion() {
#ifdef TPC_DROP_RELEASE
  return std::to_string(TPC_DROP_RELEASE());
#else
  return "";
#endif
}
...
#ifdef LLVM_TPC_COMPILER
std::string getTpcClangFullVersion() {
  std::string buf;
  llvm::raw_string_ostream OS(buf);
  OS << "TPC C/C++ compiler version " TPC_COMPILER_VERSION_STRING;
  return OS.str();
}

std::string getTpcGITClangFullVersion(){
  std::string buf;
  llvm::raw_string_ostream OS(buf);
  OS << getLLVMRevision();
  return OS.str();
