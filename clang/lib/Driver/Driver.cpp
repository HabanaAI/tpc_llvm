...
void Driver::PrintTPCGITVersion(const Compilation &C, raw_ostream &OS) const {
  OS << getTpcGITClangFullVersion() << '\n';
}
#endif
...
#ifdef LLVM_TPC_COMPILER
  if (C.getDefaultToolChain().getArch() == llvm::Triple::ArchType::tpc) {
    OS << getTpcClangFullVersion() << '\n';
  } else
#endif
  if (IsFlangMode()) {
...
bool Driver::HandleImmediateArgs(const Compilation &C) {
  ...
  #ifdef LLVM_TPC_COMPILER
  if(C.getArgs().hasArg(options::OPT__versionTPC)) {
    PrintTPCGITVersion(C, llvm::outs());
    return false;
  }
  #endif
...
#ifdef LLVM_TPC_COMPILER
            if (TC.getTriple().getArch() == llvm::Triple::tpc)
              Ty = types::TY_C;
            else
#endif
            if (CCCIsCPP())
...
#ifdef LLVM_TPC_COMPILER
      if (TC.getTriple().getArch() == llvm::Triple::tpc && Inputs.size() > 1) {
        Diag(clang::diag::err_drv_too_many_input_files);
      }
#endif
  ...
  if (!TC) {
    ...
    default:
      ...
      case llvm::Triple::tpc:
        TC = std::make_unique<toolchains::TPCToolChain>(*this, Target, Args);
        break;
#endif
      case llvm::Triple::ve:
        TC = std::make_unique<toolchains::VEToolChain>(*this, Target, Args);
