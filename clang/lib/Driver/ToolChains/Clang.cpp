...
void Clang::RenderTargetOptions(const llvm::Triple &EffectiveTriple,
  ...
  case llvm::Triple::tpc:
    AddTPCTargetArgs(Args, CmdArgs);
    break;
#endif
  }
}
...
#ifdef LLVM_TPC_COMPILER
void Clang::AddTPCTargetArgs(const ArgList &Args,
                             ArgStringList &CmdArgs) const {
}
#endif
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
...
#ifdef LLVM_TPC_COMPILER
  if (Triple.getArch() == llvm::Triple::tpc)
    CmdArgs.push_back("-fembed-bitcode=bitcode");
#endif
...
#ifdef LLVM_TPC_COMPILER
  if (Triple.getArch() == llvm::Triple::tpc) {
    StringRef CPUName = tpc::getTPCTargetCPU(Args, Triple);
    if (CPUName.equals("gaudi"))
      CmdArgs.push_back("-bfloat16");

    if (Arg *A = Args.getLastArg(options::OPT_vector_local_memory)) {
      CmdArgs.push_back("-vlm");
      CmdArgs.push_back(A->getValue());
    }
    CmdArgs.push_back("-slm");
    if (Arg *A = Args.getLastArg(options::OPT_scalar_local_memory)) {
      CmdArgs.push_back(A->getValue());
    } else {
      if (CPUName.equals("gaudi"))
        CmdArgs.push_back("1");  //kb, MAX_SLM for gaudi architecture
      else
        CmdArgs.push_back("1");  //kb, MAX_SLM for dali (goya) architecture
    }

    if (Arg *A = Args.getLastArg(options::OPT_spill_vlm)) {
      CmdArgs.push_back("-spill-vlm");
      CmdArgs.push_back(A->getValue());
    }

    CmdArgs.push_back("-mllvm");
    CmdArgs.push_back("-simplifycfg-sink-common=false");
    CmdArgs.push_back("-mllvm");
    CmdArgs.push_back("-enable-load-pre=false");

    if (Arg *A = Args.getLastArg(options::OPT_main_function)) {
      CmdArgs.push_back("-main-function");
      CmdArgs.push_back(A->getValue());
    }
    CmdArgs.push_back("-max-tensors");
    if (Arg *A = Args.getLastArg(options::OPT_max_tensors)) {
      CmdArgs.push_back(A->getValue());
    } else {
      StringRef CPUName = tpc::getTPCTargetCPU(Args, Triple);
      if (CPUName.equals("gaudi"))
        CmdArgs.push_back("16");  // MAX_TENSORS for gaudi architecture
      else
        CmdArgs.push_back("8");   // MAX_TENSORS for dali (goya) architecture
    }

    if (Args.hasArg(options::OPT_no_tpc_defs))
      CmdArgs.push_back("-no-tpc-defs");
    CmdArgs.push_back("-tpc-special");

    if (Args.hasArg(options::OPT_all_loops_taken)) {
      CmdArgs.push_back("-all-loops-taken");
    }

    if (Args.hasArg(options::OPT_reg_mem_count)) {
      CmdArgs.push_back("-reg-mem-count");
    }

    if (Args.hasArg(options::OPT_disable_lut_warn)) {
      CmdArgs.push_back("-disable-lut-warn");
    }

    if (Args.hasFlag(options::OPT_long_irf, options::OPT_no_long_irf, false)) {
      CmdArgs.push_back("-long-irf");
    }

    CmdArgs.push_back("-mllvm");
    CmdArgs.push_back("-loop-unswitch-threshold=0");
    ...
    // TPC Cost Model.
    CmdArgs.push_back("-mllvm");
    CmdArgs.push_back("-dontAnalysis=1");
  }
#endif
...
#ifdef LLVM_TPC_COMPILER
  // TPC: do not put this option in the case of TPC target, as all system files
  // must be embedded into compiler executable.
  if (Triple.getArch() != llvm::Triple::tpc) {
  ...
  if (Arg *A = Args.getLastArg(options::OPT_O_Group)) {
    ...
    if (Triple.getArch() == llvm::Triple::tpc) {
      if (A->getOption().matches(options::OPT_O0)) {
        ...
        CmdArgs.push_back("-mllvm");
        CmdArgs.push_back("-enable-misched=false");
        CmdArgs.push_back("-mllvm");
        CmdArgs.push_back("-post-RA-scheduler=false");
        CmdArgs.push_back("-mllvm");
        CmdArgs.push_back("-enable-post-misched=false");
        CmdArgs.push_back("-mllvm");
        CmdArgs.push_back("-optimize-predicates=false");
        CmdArgs.push_back("-mllvm");
        CmdArgs.push_back("-set_indx-coalescer=false");
        CmdArgs.push_back("-mllvm");
        CmdArgs.push_back("-tpc-single-packets");
        CmdArgs.push_back("-mllvm");
        CmdArgs.push_back("-tpc-debug-nops=8");
  ...
  if (Triple.getArch() == llvm::Triple::tpc) {
    ...
    CmdArgs.push_back("-Wfloat-conversion");
    CmdArgs.push_back("-Wmissing-braces");
  ...
  if (Std) {
#ifdef LLVM_TPC_COMPILER
    if ((Triple.getArch() == llvm::Triple::tpc) &&
        !StringRef(Std->getValue()).startswith("rc99") &&
        !StringRef(Std->getValue()).startswith("rc++")) {
      D.Diag(diag::err_drv_clang_unsupported)
        << (std::string(Std->getSpelling()) + " on " + Triple.str());
      CmdArgs.push_back("-std=rc99");
    } else
#endif
    ...
    if (Arg *A = Args.getLastArg(options::OPT_std_EQ, options::OPT_ansi,
      ...
      if (A != Std)
        A->render(Args, CmdArgs);
#ifdef LLVM_TPC_COMPILER
  } else if (Triple.getArch() == llvm::Triple::tpc) {
    if (types::isCXX(InputType))
      CmdArgs.push_back("-std=rc++");
    else
      CmdArgs.push_back("-std=rc99");
