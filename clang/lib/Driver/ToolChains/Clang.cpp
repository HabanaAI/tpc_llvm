...
void Clang::AddTPCTargetArgs(const ArgList &Args,
                             ArgStringList &CmdArgs) const {
  addCommonTPCTargetArgs(Args, CmdArgs);
}
#endif
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Args.hasFlag(options::OPT_fdiscard_value_names,
      ...
      // -discard-value-names prohibits external IR/MIR parsing during compilation
      // on the fly. LinkTPCHeaders pass requires IR parsing on the fly. That's why
      // forced value names discard is disabled in TPC compiler.
      //
      // It leads to slower compilation and more memory consumption due to longer
      // registers names in IR.
      Triple.getArch() != llvm::Triple::tpc) {
