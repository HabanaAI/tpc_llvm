...
#ifdef LLVM_TPC_COMPILER
  if (CI.getLangOpts().RC99) {
    llvm::IntrusiveRefCntPtr<llvm::vfs::InMemoryFileSystem>
        InMemoryFileSystem(new llvm::vfs::InMemoryFileSystem);
    llvm::IntrusiveRefCntPtr<llvm::vfs::OverlayFileSystem>
        OverlayFileSystem(new llvm::vfs::OverlayFileSystem(InMemoryFileSystem));
    llvm::IntrusiveRefCntPtr<llvm::vfs::FileSystem> RealFileSystem(
        llvm::vfs::getRealFileSystem());
    OverlayFileSystem->pushOverlay(RealFileSystem);
    registerEmbeddedLibraries(*InMemoryFileSystem, CI);
    VFS = std::move(OverlayFileSystem);
