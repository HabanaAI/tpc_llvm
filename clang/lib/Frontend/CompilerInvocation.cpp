...
#ifdef LLVM_TPC_COMPILER
  if (Args.hasArg(OPT_vector_local_memory))
    Opts.VectorLocalMemory = getLastArgIntValue(Args, OPT_vector_local_memory, 0, Diags) * 1024;
  if (Args.hasArg(OPT_scalar_local_memory))
    Opts.ScalarLocalMemory = getLastArgIntValue(Args, OPT_scalar_local_memory, 1, Diags) * 1024;
  if (Args.hasArg(OPT_spill_vlm))
    Opts.SpillVlm = getLastArgIntValue(Args, OPT_spill_vlm, 0, Diags) * 1024;
  Opts.MarkAsUsingPrintf = Args.hasArg(OPT_use_printf_in_module);
    Opts.AllLoopsTaken = Args.hasArg(OPT_all_loops_taken);
  Opts.RegMemCount = Args.hasArg(OPT_reg_mem_count);
  Opts.LutWarn = !Args.hasArg(OPT_disable_lut_warn);
  Opts.CompressInstructions = Args.hasFlag(OPT_instr_compress, OPT_no_instr_compress);
  Opts.LongIRF = Args.hasFlag(OPT_long_irf, OPT_no_long_irf, false);
  Opts.TpcDnorm = Args.hasFlag(OPT_tpc_dnorm, OPT_no_tpc_dnorm, false);
  Opts.MaxTensor = getLastArgIntValue(Args, OPT_max_tensors, 16, Diags);
  
  Opts.CompileForLibrary = Args.hasArg(OPT_tpc_library);
#endif
...
#ifdef LLVM_TPC_COMPILER
  // Adjust options for restricted C99 for TPC target.
  if (LangStd == LangStandard::Kind::lang_rc99) {
    Opts.RC99 = true;
    Opts.DollarIdents = false;
  }
  if (LangStd == LangStandard::Kind::lang_rcxx) {
    Opts.RC99 = true;
    Opts.RCXX = true;
    Opts.DollarIdents = false;
  }
...
#ifdef LLVM_TPC_COMPILER
  Opts.MainFunction = Args.getLastArgValue(OPT_main_function, "main").str();
  // Default option for "-max-tensors" is provided only to avoid test failures.
  // Actual default value depends on target architecture and is set by driver.
  Opts.NumTensors = getLastArgIntValue(Args, OPT_max_tensors, 8, Diags);
  Opts.SlmMaxSize = getLastArgIntValue(Args, OPT_scalar_local_memory, 1024, Diags);
  Opts.VlmMaxSize = getLastArgIntValue(Args, OPT_vector_local_memory, 80*1024, Diags);
  Opts.BFloat16 = Args.hasFlag(OPT_bfloat16, OPT_no_bfloat16, false);
  Opts.Float8 = Args.hasFlag(OPT_float8, OPT_no_float8, false);
  Opts.LongIRF = Args.hasFlag(OPT_long_irf, OPT_no_long_irf, false);
  Opts.UsePrintf = Args.hasArg(OPT_use_printf_in_module);
  Opts.CompileForLibrary = Args.hasArg(OPT_tpc_library);
#endif
}
...
static void ParseTargetArgs(TargetOptions &Opts, ArgList &Args,
                            DiagnosticsEngine &Diags) {
#ifdef LLVM_TPC_COMPILER
  if (StringRef(Opts.Triple).startswith("tpc") && Opts.CPU.empty())
    Opts.CPU = "goya";
#endif
...
#ifdef LLVM_TPC_COMPILER
  if (T.getArch() == llvm::Triple::tpc)
    llvm::TPCCompilerArguments.append(
        llvm::makeTPCCompilerArgumentsString(CommandLineArgs));

  if (LangOpts.RC99) {
    std::shared_ptr<PreprocessorOptions> PPOpts = Res.getPreprocessorOptsPtr();
    bool IncludeTpcSpecial = Args.hasArg(OPT_tpc_special);
    if (!Args.hasArg(OPT_no_tpc_defs)) {
      ArrayRef<EmbeddedFile> EmbeddedFiles = getEmbeddedFiles();
      for (const EmbeddedFile &F : EmbeddedFiles) {
        PPOpts->Includes.push_back(F.Name.str());
        auto P = std::make_pair(F.Name.str(), F.Content);
        PPOpts->RemappedFileBuffers.push_back(P);
      }
    } else
      IncludeTpcSpecial = true;
    if (IncludeTpcSpecial)
      PPOpts->addMacroDef("INCLUDE_TPC_SPECIAL_H");

    if (Res.getTargetOpts().CPU == "gaudi") {
      LangOpts.BFloat16 = 1;
    }
    if (Res.getTargetOpts().CPU == "goya2" ||
        Res.getTargetOpts().CPU == "greco" ||
        Res.getTargetOpts().CPU == "gaudib") {
      LangOpts.BFloat16 = 1;
      LangOpts.Half = 1;
    ...
    if (Res.getTargetOpts().CPU == "gaudi2" ||
        Res.getTargetOpts().CPU == "doron1") {
      LangOpts.BFloat16 = 1;
      LangOpts.Float8 = 1;
      LangOpts.Half = 1;
      LangOpts.HalfArgsAndReturns = 1;
