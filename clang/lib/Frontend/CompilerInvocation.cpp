...
#ifdef LLVM_TPC_COMPILER
  Opts.MainFunction = Args.getLastArgValue(OPT_main_function, "main");
  ...
  if (LangOpts.RC99) {
    ...
    if (!Args.hasArg(OPT_no_tpc_defs)) {
      ...
      for (const EmbeddedFile &F : EmbeddedFiles) {
        PPOpts->Includes.push_back(F.Name);
        auto P = std::make_pair(F.Name, F.Content);
    ...
    Res.originalArgs.reserve(CommandLineArgs.size());
    for (const char* Arg : CommandLineArgs){
      // Null pointer is possible
      if (Arg)
        Res.originalArgs.push_back(Arg);
    }
  }
#endif
